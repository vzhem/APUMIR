...
$StatePath = Join-Path $env:TEMP "apu-max-test-rank-connected-phones-recovery-01-state.json"
$LogDirectory = Join-Path $env:TEMP "apu-max-test-rank-connected-phones-recovery-01-logs"
...
if ((Test-Path $StatePath) -or (Test-Path $LogDirectory)) {
    throw "Max test-rank recovery already attempted; preserve evidence"
}
...
Write-Host "MAX TEST RANK CONNECTED PHONES RECOVERY PASS"
