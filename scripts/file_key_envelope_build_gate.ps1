...
$ExpectedSource = "6c7586c"
$OldNative = "81384E504737D9531ABC6C81D70F3225B2D30574C23E00587EBD5772779AD790"
$OldBinding = "8C0BB329466B44ECDE6F5DFFD8212EC7CBF69C1BC0696AFB752BE87551AD9304"
...
$Prefix = Join-Path $env:TEMP "apu-file-key-envelope-build"
...
if ($Existing.Count) { throw "File key envelope build already attempted; preserve evidence" }
...
try {
    ...
    $Normalize=Join-Path $env:TEMP "apu-file-key-envelope-normalize.py"
    @'
...
'@ | Set-Content $Normalize -Encoding ASCII
    ...
    foreach($Marker in @("createFileKeyEnvelope","openFileKeyEnvelope")){
        if($Text -notmatch $Marker){throw "Missing binding marker: $Marker"}
    }
    $env:JAVA_HOME=$Jdk;$env:Path="$Jdk\bin;"+$env:Path;$env:GITHUB_REF_NAME="v11.16.50"
    ...
    [ordered]@{schema=1;purpose="Authenticated per-transfer file key envelope build";outcome=$Outcome;failure=$Failure;sourceCommit=$ExpectedSource;oldNative=$OldNative;newNative=$NewNative;oldBinding=$OldBinding;newBinding=$NewBinding;apkSize=$ApkSize;apkSha256=$ApkHash;contactPinningSourceReady=$true;transportWired=$false;adbUsed=$false;phonesChanged=$false}|ConvertTo-Json -Depth 5|Set-Content $StatePath -Encoding UTF8
    $Hash=(Get-FileHash $StatePath -Algorithm SHA256).Hash
    Write-Host "`nOutcome: $Outcome";Write-Host "State: $StatePath";Write-Host "State SHA256: $Hash";Write-Host "Native: $NewNative";Write-Host "Binding: $NewBinding";Write-Host "APK: $ApkSize / $ApkHash";Write-Host "Contact pinning source/transport/ADB/phones: True/False/False/False"
}
if($Outcome -ne "PASS"){throw "File key envelope build failed"}
Write-Host "FILE KEY ENVELOPE BUILD PASS"
