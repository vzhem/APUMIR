...
$ExpectedSource = "a8a849e9de5057561cee6944201078f5c950de5a"
...
$Prefix = Join-Path $env:TEMP "apu-file-transfer-f1-chunk-store-build"
$StatePath = "$Prefix-state.json"
$GradleLog = "$Prefix-gradle.log"

$Existing = @(@($StatePath, $GradleLog) | Where-Object { Test-Path $_ })
if ($Existing.Count -ne 0) { throw "Chunk store build already attempted; preserve evidence" }
...
try {
    ...
    try {
        ...
        if ($Code -ne 0) { throw "Gradle chunk store test/build failed: $Code" }
    ...
    [ordered]@{
        schema = 1
        purpose = "File F1 bounded encrypted chunk store JVM and APK build"
        outcome = $Outcome
        failure = $Failure
        sourceCommit = $ExpectedSource
        jvmTestClass = "FileTransferChunkStoreTest"
        appApkSize = $AppSize
...
if ($Outcome -ne "PASS") { throw "File F1 chunk store build failed" }
Write-Host "FILE TRANSFER F1 CHUNK STORE BUILD PASS"
