...
$ExpectedSource = "53dd4fb3f1e7646d041c52867b14cf005ba0ecdc"
...
$Prefix = Join-Path $env:TEMP "apu-file-transfer-f1-key-vault-build"
$StatePath = "$Prefix-state.json"
$GradleLog = "$Prefix-gradle.log"

$Existing = @(@($StatePath, $GradleLog) | Where-Object { Test-Path $_ })
if ($Existing.Count -ne 0) { throw "File key vault build already attempted; preserve evidence" }
...
try {
    ...
    try {
        ...
        if ($Code -ne 0) { throw "Gradle key vault test/build failed: $Code" }
    ...
    [ordered]@{
        schema = 1
        purpose = "File F1 device-bound transfer key vault JVM and instrumentation build"
        outcome = $Outcome
        failure = $Failure
        sourceCommit = $ExpectedSource
        envelopeJvmTestsRun = $true
    ...
    Write-Host "Production key/transport/ADB/phones: False/False/False/False"
}
if ($Outcome -ne "PASS") { throw "File F1 key vault build failed" }
Write-Host "FILE TRANSFER F1 KEY VAULT BUILD PASS"
