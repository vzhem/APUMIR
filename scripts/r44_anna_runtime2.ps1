...
$OldRuntimeStatePath = Join-Path $env:TEMP "apu-r4.4-dual-v11.16.15-anna-runtime.json"
$ExpectedOldRuntimeStateHash = "FF4F61A3320A5A78B344C9E344A04E69FF34CCC3275CC37E519E73E64BEB7968"
$VisibilityStdoutPath = Join-Path $env:TEMP "apu-r4.4-anna-adb-visibility1.stdout.log"
$VisibilityStderrPath = Join-Path $env:TEMP "apu-r4.4-anna-adb-visibility1.stderr.log"
$ExpectedVisibilityStdoutHash = "0E481BBCDDABC8556ACEA4E8720014BA2289B656678230E5E9EDCD8296A35E05"
$ExpectedVisibilityStderrHash = "E3B0C44298FC1C149AFBF4C8996FB92427AE41E4649B934CA495991B7852B855"
$StatePath = Join-Path $env:TEMP "apu-r4.4-dual-v11.16.15-anna-runtime2.json"
$EvidenceDir = Join-Path $env:TEMP "apu-r4.4-dual-v11.16.15-anna-runtime2-evidence"
...
foreach ($RequiredPath in @(
    ...
    $OldRuntimeStatePath,
    $VisibilityStdoutPath,
    $VisibilityStderrPath
)) {
...
$OldRuntimeStateHash = (Get-FileHash -LiteralPath $OldRuntimeStatePath -Algorithm SHA256).Hash
if ($OldRuntimeStateHash -ne $ExpectedOldRuntimeStateHash) {
    throw "Old runtime state hash mismatch: $OldRuntimeStateHash"
}
$OldRuntimeState = Get-Content -LiteralPath $OldRuntimeStatePath -Raw | ConvertFrom-Json
$OldRuntimeStoppedPreAction = $OldRuntimeState.outcome -eq "INCOMPLETE_DO_NOT_REPEAT"
$OldRuntimeStoppedPreAction = $OldRuntimeStoppedPreAction -and $OldRuntimeState.installStarted -eq $false
$OldRuntimeStoppedPreAction = $OldRuntimeStoppedPreAction -and $OldRuntimeState.launchStarted -eq $false
$OldRuntimeStoppedPreAction = $OldRuntimeStoppedPreAction -and $OldRuntimeState.zhenyaTouched -eq $false
$OldRuntimeStoppedPreAction = $OldRuntimeStoppedPreAction -and $OldRuntimeState.stasTouched -eq $false
$OldRuntimeStoppedPreAction = $OldRuntimeStoppedPreAction -and $OldRuntimeState.userPayloadPublished -eq $false
$OldRuntimeStoppedPreAction = $OldRuntimeStoppedPreAction -and $OldRuntimeState.automaticRetry -eq $false
if (-not $OldRuntimeStoppedPreAction) {
    throw "Old runtime state does not prove a pre-install/pre-launch stop"
}

$VisibilityStdoutHash = (Get-FileHash -LiteralPath $VisibilityStdoutPath -Algorithm SHA256).Hash
$VisibilityStderrHash = (Get-FileHash -LiteralPath $VisibilityStderrPath -Algorithm SHA256).Hash
$VisibilityHashesValid = $VisibilityStdoutHash -eq $ExpectedVisibilityStdoutHash
$VisibilityHashesValid = $VisibilityHashesValid -and $VisibilityStderrHash -eq $ExpectedVisibilityStderrHash
if (-not $VisibilityHashesValid) {
    throw "Anna visibility evidence hash mismatch"
}
$VisibilityStdoutLines = @(Get-Content -LiteralPath $VisibilityStdoutPath)
$VisibilityStderrLines = @(Get-Content -LiteralPath $VisibilityStderrPath)
$SavedAnnaReadyPattern = "^{0}\s+device(?:\s|$)" -f [regex]::Escape($AnnaSerial)
$SavedAnnaReadyCount = @(
    $VisibilityStdoutLines | Where-Object { $_ -match $SavedAnnaReadyPattern }
).Count
if ($SavedAnnaReadyCount -ne 1 -or $VisibilityStderrLines.Count -ne 0) {
    throw "Saved visibility evidence does not prove exact Anna device readiness"
}
...
try {
    ...
    $State = [ordered]@{
        schema = 1
        purpose = "approved corrected Anna-only r4.4 dual-session v11.16.15 runtime2"
        outcome = $Outcome
        failure = $Failure
        completedUtc = (Get-Date).ToUniversalTime().ToString("o")
        userApproved = $true
        oldRuntimeState = $OldRuntimeStatePath
        oldRuntimeStateSha256 = $OldRuntimeStateHash
        oldRuntimeInstallStarted = $OldRuntimeState.installStarted
        oldRuntimeLaunchStarted = $OldRuntimeState.launchStarted
        visibilityStdout = $VisibilityStdoutPath
        visibilityStdoutSha256 = $VisibilityStdoutHash
        visibilityStderr = $VisibilityStderrPath
        visibilityStderrSha256 = $VisibilityStderrHash
        savedAnnaReadyCount = $SavedAnnaReadyCount
...
Write-Host "R4.4 ANNA DUAL-SESSION RUNTIME2 PASS"
