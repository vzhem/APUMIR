...
try {
    ...
    if (-not $Python) {
        ...
        & $Python @PyArgs (Join-Path $RepoRoot 'tools\sandbox\check_room_schema.py') `
            (Join-Path $DataLocal 'AppDatabase.kt') (Join-Path $DataLocal 'entity') 'MIGRATION_16_17'
    ...
    Write-Output ''
    Write-Output '===== step 1: unit tests (groups, rank policy, link parsers, referrals, calls, peer rating, sealing, onboarding, receipts, hearts, post views) ====='
    & $Gradlew --console=plain :app:testDebugUnitTest `
        ...
        --tests 'com.vladimir.messenger.data.channel.*' `
