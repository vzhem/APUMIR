...
$ExpectedSource = "d8bfab4c355ae7e7485347639ac0b6f8ba84f9e7"
...
$Prefix = Join-Path $env:TEMP "apu-file-transfer-rank-policy-build"
$StatePath = "$Prefix-state.json"
$GradleLog = "$Prefix-gradle.log"

$Existing = @(@($StatePath, $GradleLog) | Where-Object { Test-Path $_ })
if ($Existing.Count -ne 0) { throw "Rank policy build already attempted; preserve evidence" }
...
try {
    ...
    try {
        ...
        if ($Code -ne 0) { throw "Gradle rank policy test/build failed: $Code" }
    ...
    [ordered]@{
        schema = 1
        purpose = "Referral-rank outbound media entitlement policy build"
        outcome = $Outcome
        failure = $Failure
        sourceCommit = $ExpectedSource
        rankPolicyTestsRun = $true
        packetCodecRegressionRun = $true
        appApkSize = $AppSize
    ...
    Write-Host "Referral runtime/UI/transport/ADB/phones: False/False/False/False/False"
}
if ($Outcome -ne "PASS") { throw "Rank policy build failed" }
Write-Host "FILE TRANSFER RANK POLICY BUILD PASS"
