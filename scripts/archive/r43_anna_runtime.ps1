...
$StatePath = Join-Path $env:TEMP "apu-r4.3-observe-v11.16.14-anna-runtime.json"
$EvidenceDir = Join-Path $env:TEMP "apu-r4.3-observe-v11.16.14-anna-runtime-evidence"
...
foreach ($Path in @($Adb,$ApkPath,$ApkStatePath,$Runtime13StatePath)) {
...
function Invoke-Captured {
    ...
    $P = Start-Process -FilePath $File -ArgumentList $Args -WorkingDirectory $RepoRoot `
...
function Native-Lines {
    param([object]$Device,[string]$Pid,[int64]$Epoch,[string]$Phase)
    ...
    $Result=[System.Collections.Generic.List[string]]::new(); $Pattern="^\s*(\d+\.\d+)\s+{0}\s+" -f [regex]::Escape($Pid)
