...
$ExpectedSource = "5d57afff9d4aff2514581573ed59942ccfa8957c"
...
$Prefix = Join-Path $env:TEMP "apu-referral-r1-link-codec-build"
$StatePath = "$Prefix-state.json"
$GradleLog = "$Prefix-gradle.log"

$Existing = @(@($StatePath, $GradleLog) | Where-Object { Test-Path $_ })
if ($Existing.Count -ne 0) { throw "R1 link codec gate already attempted; preserve evidence" }
...
if ((Get-FileHash $NativePath -Algorithm SHA256).Hash -ne $ExpectedNative) {
    throw "Native baseline mismatch"
}
if ((Get-FileHash $BindingPath -Algorithm SHA256).Hash -ne $ExpectedBinding) {
    throw "Binding baseline mismatch"
}
...
try {
    ...
    [ordered]@{
        schema = 1
        purpose = "R1 strict HTTPS referral link codec JVM and APK gate"
        outcome = $Outcome
        failure = $Failure
    ...
    Write-Host "JVM tests/ADB/phones: True/False/False"
}
if ($Outcome -ne "PASS") { throw "R1 link codec gate failed" }
Write-Host "REFERRAL R1 LINK CODEC BUILD PASS"
