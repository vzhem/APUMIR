...
$FailedState = Join-Path $env:TEMP "apu-file-transfer-f0-build-state.json"
$ExpectedFailedState = "059A13A9763411C5BE68BDBD7EE6E6A1D3A2BED3A269C86BDE5EF7A77755AFBA"
$Jdk = "C:\Program Files\Android\Android Studio\jbr"
$Prefix = Join-Path $env:TEMP "apu-file-transfer-f0-production-recovery-01"
$StatePath = "$Prefix-state.json"
$RustLog = "$Prefix-rust.log"
$GradleLog = "$Prefix-gradle.log"

$Existing = @(@($StatePath, $RustLog, $GradleLog) | Where-Object { Test-Path $_ })
if ($Existing.Count -ne 0) { throw "File F0 recovery already attempted; preserve evidence" }
if (-not (Test-Path $FailedState) -or
    (Get-FileHash $FailedState -Algorithm SHA256).Hash -ne $ExpectedFailedState) {
    throw "Failed typecheck evidence mismatch"
}
$Failed = Get-Content $FailedState -Raw | ConvertFrom-Json
if ($Failed.outcome -ne "INCOMPLETE_DO_NOT_REPEAT" -or $Failed.adbUsed -ne $false) {
    throw "Unexpected failed-gate state"
}
...
try {
    ...
    [ordered]@{
        schema = 1
        purpose = "Secure File Transfer F0 production recovery after missing host linker"
        outcome = $Outcome
        failure = $Failure
        sourceCommit = $ExpectedSource
        priorFailedStateSha256 = $ExpectedFailedState
        hostTestsRun = $false
        hostTestBlocker = "MSVC link.exe unavailable"
...
if ($Outcome -ne "PASS") { throw "File transfer F0 production recovery failed" }
Write-Host "FILE TRANSFER F0 PRODUCTION RECOVERY PASS"
