...
$StatePath = Join-Path $env:TEMP "apu-r4.3-observe-v11.16.14-apk-build.json"
$LogPath = Join-Path $env:TEMP "apu-r4.3-observe-v11.16.14-apk-build.log"
$StdoutPath = Join-Path $env:TEMP "apu-r4.3-observe-v11.16.14-apk-build.stdout.log"
$StderrPath = Join-Path $env:TEMP "apu-r4.3-observe-v11.16.14-apk-build.stderr.log"
$JavaStdoutPath = Join-Path $env:TEMP "apu-r4.3-observe-v11.16.14-java.stdout.log"
$JavaStderrPath = Join-Path $env:TEMP "apu-r4.3-observe-v11.16.14-java.stderr.log"
$AaptStdoutPath = Join-Path $env:TEMP "apu-r4.3-observe-v11.16.14-aapt.stdout.log"
$AaptStderrPath = Join-Path $env:TEMP "apu-r4.3-observe-v11.16.14-aapt.stderr.log"
$SignerStdoutPath = Join-Path $env:TEMP "apu-r4.3-observe-v11.16.14-signer.stdout.log"
$SignerStderrPath = Join-Path $env:TEMP "apu-r4.3-observe-v11.16.14-signer.stderr.log"
$StableApkPath = Join-Path $env:TEMP "apu-r4.3-observe-v11.16.14.apk"
...
if ($Existing.Count -gt 0) {
    throw "R4.3 V11.16.14 APK BUILD ALREADY ATTEMPTED - DO NOT REPEAT: $($Existing -join ', ')"
}
foreach ($RequiredPath in @($SoPath, $JavaExe, $SdkPath, $Gradlew, $PowerShellExe)) {
...
try {
    ...
    $JavaProcess = Start-Process `
        ...
        -PassThru
    $JavaExitCode = Wait-ExactProcess -Process $JavaProcess -TimeoutMilliseconds 30000 -Label "Java version"
...
"@
    ...
    $BuildExitCode = Wait-ExactProcess -Process $BuildProcess -TimeoutMilliseconds 900000 -Label "Gradle build"
    ...
    $AaptProcess = Start-Process `
        ...
        -PassThru
    $AaptExitCode = Wait-ExactProcess -Process $AaptProcess -TimeoutMilliseconds 30000 -Label "AAPT"
...
"@
    ...
    $SignerExitCode = Wait-ExactProcess -Process $SignerProcess -TimeoutMilliseconds 60000 -Label "APK signer"
    ...
    $State = [ordered]@{
        schema = 1
        purpose = "r4.3 observe-only v11.16.14 APK artifact build"
        outcome = $Outcome
        failure = $Failure
