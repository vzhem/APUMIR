...
$ExpectedSource = "d2e2e2dad20ee4f3c6bf0151a0545de0b521fb8e"
...
$FailedState = Join-Path $env:TEMP "apu-file-transfer-f2-anna-stas-direct-tunnel-02-state.json"
$FailedCleanup = Join-Path $env:TEMP "apu-file-transfer-f2-anna-stas-direct-tunnel-02-cleanup.log"
$FailedReceiver = Join-Path $env:TEMP "apu-file-transfer-f2-anna-stas-direct-tunnel-02-receiver.stdout.log"
$ExpectedFailedState = "6EA472D0D78821D9165CEF2E5B90DC9ED7AA7BB6A72821235EF461EA2E788CD2"
$ExpectedFailedCleanup = "4D91F0DF9F768C64A124F469428E05184917EBD66CCD1A5DE94AAAA753C9520F"
$ExpectedFailedReceiver = "05C4950AF4D81A8AD6602E55DB52FDF9626FC8FCA4663CFEC93FEEC140A7214A"
$Jdk = "C:\Program Files\Android\Android Studio\jbr"
$Prefix = Join-Path $env:TEMP "apu-file-transfer-f2-socket-tunnel-build"
$StatePath = "$Prefix-state.json"
$GradleLog = "$Prefix-gradle.log"

$Existing = @(@($StatePath, $GradleLog) | Where-Object { Test-Path $_ })
if ($Existing.Count -ne 0) { throw "Socket tunnel build already attempted; preserve evidence" }
if ((Get-FileHash $FailedState -Algorithm SHA256).Hash -ne $ExpectedFailedState -or
    ...
    throw "Direct-tunnel failure evidence mismatch"
}
...
try {
    ...
    try {
        ...
        if ($Code -ne 0) { throw "Gradle socket tunnel test build failed: $Code" }
    ...
    [ordered]@{
        schema = 1
        purpose = "File F2 Anna-Stas raw socket ADB tunnel harness build"
        outcome = $Outcome
        failure = $Failure
        sourceCommit = $ExpectedSource
        failedTunnelStateSha256 = $ExpectedFailedState
        appApkSha256 = $ExpectedApp
        testApkSize = $TestSize
        testApkSha256 = $TestHash
        rustNetworkEngineUsed = $false
    ...
    Write-Host "Rust network/production network/ADB/phones: False/False/False/False"
}
if ($Outcome -ne "PASS") { throw "Socket tunnel build failed" }
Write-Host "FILE TRANSFER F2 SOCKET TUNNEL BUILD PASS"
