...
$BuildState = Join-Path $env:TEMP "apu-file-transfer-f2-direct-tunnel-build-state.json"
$StatePath = Join-Path $env:TEMP "apu-file-transfer-f2-anna-stas-direct-tunnel-02-state.json"
$CleanupLog = Join-Path $env:TEMP "apu-file-transfer-f2-anna-stas-direct-tunnel-02-cleanup.log"
$SenderLog = Join-Path $env:TEMP "apu-file-transfer-f2-anna-stas-direct-tunnel-02-sender.log"
$ReceiverOut = Join-Path $env:TEMP "apu-file-transfer-f2-anna-stas-direct-tunnel-02-receiver.stdout.log"
$ReceiverErr = Join-Path $env:TEMP "apu-file-transfer-f2-anna-stas-direct-tunnel-02-receiver.stderr.log"
$ReceiverLog = Join-Path $env:TEMP "apu-file-transfer-f2-anna-stas-direct-tunnel-02-receiver.log"

$ExpectedBuildState = "990BEDB6D290C31B647F7CCC6F42EB363CC223DB0F950D1824545ED36B8907FA"
$ExpectedApp = "33CFC80617269318C711632F04E299511254CD75598C4C68061AFD7023582B52"
$ExpectedTest = "63C0C3E5F57AF5079F3F50CB625205CC90CB6590D6629581BE2145DDC6B1BEAC"
$HostForwardPort = 47778
$AnnaReversePort = 37778
$StasTcpPort = 7778
...
if (@($Evidence | Where-Object { Test-Path $_ }).Count -ne 0) {
    throw "Direct tunnel phone gate already attempted; preserve evidence"
}
if ((Get-FileHash $BuildState -Algorithm SHA256).Hash -ne $ExpectedBuildState -or
    ...
    throw "Direct tunnel evidence/APK mismatch"
}
...
$Result = [ordered]@{
    ...
    transport = "ADB-tunneled-direct-TCP-test-only"
    annaReplaceInstall = $true; stasReplaceInstall = $true
    testKeyOutOfBand = $true; productionNetworkPath = $false
    productionKeyExchange = $false; productionUi = $false
...
try {
    ...
    & $Adb -s $Stas forward "tcp:$HostForwardPort" "tcp:$StasTcpPort" | Out-Host
    if ($LASTEXITCODE -ne 0) { throw "Stas adb forward failed" }
    & $Adb -s $Anna reverse "tcp:$AnnaReversePort" "tcp:$HostForwardPort" | Out-Host
    if ($LASTEXITCODE -ne 0) { throw "Anna adb reverse failed" }

    $ReceiverArguments = @(
        "-s", $Stas, "shell", "am", "instrument", "-w", "-r",
        "-e", "class", $ReceiverClass, "-e", "file_run_id", $RunId,
        "-e", "expected_sender", $AnnaBefore.nodeValue,
        "-e", "file_test_key", $KeyText, $Runner
    )
    ...
    if (-not $Ready) { throw "Receiver did not become ready" }
    ...
    if (-not $ReceiverProcess.WaitForExit(120000)) { throw "Direct receiver timeout" }
    ...
    if ($ReceiverProcess.ExitCode -ne 0 -or $ReceiverText -notmatch "OK \(1 test\)" -or
        ...
        throw "Direct receiver instrumentation failed"
    }
    ...
    $Result.encryptedFileCrossDeviceRoundTrip = $true
    $Result.encryptedPngCrossDeviceRoundTrip = $true
    $Result.fileSha256Verified = $true; $Result.pngDecoded = $true
...
Write-Host "Production network/key exchange/UI: False/False/False"
Write-Host "DB delete/data clear/force-stop: False/False/False"
Write-Host "FILE TRANSFER F2 ANNA-STAS DIRECT TUNNEL PASS"
