...
$State = Join-Path $env:TEMP "apu-referral-r1-pending-stas-phone-recovery-01-state.json"
$Log = Join-Path $env:TEMP "apu-referral-r1-pending-stas-phone-recovery-01.log"
...
if ($Existing.Count -ne 0) { throw "Stas pending phone recovery already attempted; preserve evidence" }
...
if (-not $PackageVisible -or $VersionBefore -ne "versionName=v11.16.28") {
...
Write-Host "REFERRAL R1 PENDING STAS PHONE RECOVERY PASS"
