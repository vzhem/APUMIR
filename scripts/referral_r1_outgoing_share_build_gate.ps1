...
$ExpectedSource = "60269342105ce837ccb91aff2ff575e335fcb719"
...
$Prefix = Join-Path $env:TEMP "apu-referral-r1-outgoing-share-build"
$StatePath = "$Prefix-state.json"
$GradleLog = "$Prefix-gradle.log"

$Existing = @(@($StatePath, $GradleLog) | Where-Object { Test-Path $_ })
if ($Existing.Count -ne 0) { throw "Outgoing share gate already attempted; preserve evidence" }
...
try {
    ...
    [ordered]@{
        schema = 1
        purpose = "R1 outgoing signed referral share compile"
        outcome = $Outcome
        failure = $Failure
    ...
    Write-Host "Public deployment/ADB/phones: False/False/False"
}
if ($Outcome -ne "PASS") { throw "Outgoing share gate failed" }
Write-Host "REFERRAL R1 OUTGOING SHARE BUILD PASS"
