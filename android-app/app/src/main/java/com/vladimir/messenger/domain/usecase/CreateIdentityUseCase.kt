    ...
    suspend operator fun invoke(name: String): Result<IdentityResult> {
        return try {
            ...
            prefs.edit()
                ...
                // Без приватного ключа ядро при следующем старте сочло бы, что
                // ключей нет, и выдало НОВЫЙ адрес: QR, показанный сразу после
                // регистрации, указывал бы на узел, которого уже не существует,
                // и сообщения уходили в никуда. Сервис пишет то же значение при
                // старте - здесь оно должно появиться раньше, до первого QR.
                .putString("existing_private_key", publicKey)
                .apply()
