...
class FileTransferChunkStore(
    ...
    fun storeEncryptedChunk(
        ...
        val temporary = checkedChild(chunks, ".${chunkFileName(chunkIndex)}.${UUID.randomUUID()}.tmp")
        try {
            FileOutputStream(temporary).use { output ->
                output.write(ciphertext)
                output.fd.sync()
            }
            check(temporary.length() == ciphertext.size.toLong()) { "Incomplete encrypted chunk write" }
            try {
                Files.move(
                    temporary.toPath(),
                    target.toPath(),
                ...
                throw IllegalStateException("Atomic encrypted chunk move unavailable", unsupported)
            }
    ...
    private fun chunksDirectory(transferId: String, create: Boolean): File {
        ...
        val chunks = checkedChild(transfer, "chunks")
        if (create) {
    ...
    private fun readBounded(file: File): ByteArray {
        ...
        check(length in MIN_CIPHERTEXT_BYTES.toLong()..MAX_CIPHERTEXT_BYTES.toLong()) {
            "Stored encrypted chunk has invalid size"
        }
        return file.inputStream().use { input ->
            ...
            while (offset < bytes.size) {
                ...
                check(read > 0) { "Stored encrypted chunk truncated while reading" }
                offset += read
            }
            check(input.read() == -1) { "Stored encrypted chunk grew while reading" }
            bytes
        }
    }
