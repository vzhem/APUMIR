...
///
/// K4: клонируется (канал команд + общий `Arc`), поэтому движок может
/// держать свою копию в отдельном потоке presence и в то же время отдавать
/// `Option<DirectTransport>` из-под замка в FFI-вызовах.
#[derive(Clone)]
