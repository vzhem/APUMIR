...
rust_core_path = '/workspace/rust-core/src/network'

content = '''//! Store-and-Forward Module for Offline Message Delivery
...
os.makedirs(rust_core_path, exist_ok=True)
with open(f'{rust_core_path}/store_and_forward.rs', 'w') as f:
    f.write(content)
print('Created store_and_forward.rs')

mod_rs = f'{rust_core_path}/mod.rs'
if os.path.exists(mod_rs):
    with open(mod_rs, 'r') as f:
        c = f.read()
    if 'pub mod store_and_forward' not in c:
        with open(mod_rs, 'w') as f:
            f.write('pub mod store_and_forward;\n' + c)
        print('Updated mod.rs')
else:
    with open(mod_rs, 'w') as f:
        f.write('pub mod store_and_forward;')
    print('Created mod.rs')
