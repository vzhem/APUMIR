...
Что из этого сделано (v11.70.13, Android-слой, без FFI): один хранитель на
файл, выбранный отправителем из контактов в сети по рейтингу; хранитель
принимает только от своих контактов, в пределах квоты «Место под
пересылку», с потолками 32 файла / 8 от одного отправителя; хранит
шифротекст, манифест, конверт ключа (запечатан для получателя) и
подписанную привязку отправителя; отдаёт получателю при появлении;
получатель проверяет отправителя по манифесту и привязке (TOFU-закрепление
на `originId`); подтверждения - `CUSTODY_ACK` по цепочке, итоговый обычный
`ACK` - отправителю; TTL = min(срок манифеста, 7 дней), брошенное - 48 ч.
Подробно и как проверить - `CHANNEL_SWARM_DESIGN.md` §9.2 «Хранение у
третьего телефона» и §9.4 «Этап 7». С v11.70.18 (этап 8): до двух
держателей на файл (по одному за раз), получатель берёт куски от любого
проверенного держателя и делит недостающее между ними инвентарём
`CUSTODY_WANT`; копии отпускаются итоговым подтверждением получателя всем
держателям и `ACK_RELEASE` от отправителя - §9.4 «Этап 8». **Не сделано:**
подписанные квитанции хранения (custody receipt), Merkle-привязка кусков
(хранитель проверяет только геометрию и размер), at-rest ключ поверх
шифротекста, приём от чужаков, раздача сидами по `transferId`.
