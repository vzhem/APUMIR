...
третьего телефона» и §9.4 «Этап 7». **Не сделано:** подписанные квитанции
хранения (custody receipt), несколько держателей и инвентарь недостающих
кусков, Merkle-привязка кусков (хранитель проверяет только геометрию и
размер), at-rest ключ поверх шифротекста, приём от чужаков.
