...
> **СТАТУС 2026-08-30: это проектная памятка, а не описание работающих
> скриптов.** Раскладка с `RESTORE_APU.ps1` и `VERIFY_BACKUP.ps1` ниже
> реализована НЕ была — таких файлов в репозитории нет (`git ls-tree -r
> --name-only origin/main scripts/`). Рабочие скрипты сегодня:
> `scripts/backup-to-flash.ps1` (быстрое зеркало рабочего дерева) и
> `scripts/backup-to-usb.ps1` + `scripts/restore-from-usb.ps1` (полная копия с
> sha256-манифестом и восстановлением). Требования ниже к содержимому копии,
> метке `APU_BACKUP` и проверке целостности остаются в силе и в этих скриптах
> выполнены; имена файлов и дерево папок — устарели.
