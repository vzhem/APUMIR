...
| 5.13 Снятие заморозки ядра: CI перегенерирует uniffi-мост из `lib.udl` (просьба владельца 2026-09-15 «ядро самое лучшее») | 🟡 | `scripts/ci/build-release.yml` (шаги Regenerate/Upload/Push back), первая функция `core_build_info()` + строка «Ядро» в настройках — v11.70.23 (готовится). Ждёт копирования workflow владельцем. План — `docs/CORE_ROADMAP.md`; код — `AI_HANDOFF.md`, пункт 0s |
