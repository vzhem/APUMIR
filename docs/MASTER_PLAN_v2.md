...
| Друзей | Статус | Отправка media / лимит файла* | Группы и каналы |
|---:|---|---|---|
...
| 20 | **Организатор** | до 50 MiB | создание групп |
...
Создание группы открывается строго с 10 qualified direct referrals (`Проводник`), создание канала —
с 30 (`Навигатор`). Проверка должна быть не только в UI: use case/repository обязан повторно проверять
entitlement, чтобы кнопку нельзя было обойти deep link, старым клиентом или прямым вызовом API.
