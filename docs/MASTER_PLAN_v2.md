...
| 5.18 Ядро, этап K4-1: presence «своим» и редкий маяк вместо объявления всем (меньше трафика в большой сети) | 🔧 код готов | 2026-09-17, ветка `arena/01a0b097-apumir`: модуль `presence_scope.rs` (маяк раз в 10 мин, «своим» лично раз в минуту по QUIC, порог 50 узлов), приём кадров `ppres`, `set_presence_audience` в мосте, CI-проверка компиляции ядра на PR (`.github/workflows/ci-core-check.yml`). В маленькой сети поведение прежнее. Карта — `AI_HANDOFF.md` 0w, план — `CORE_ROADMAP.md` K4-1. В CI не собрано, на телефонах не проверено |
...
| K4 | Presence и поиск узлов без линейного роста: presence только «своим», DHT наружу, свой rendezvous/relay на Cloudflare Worker (таймаут + фолбэк обязательны) | 🔧 K4-1 код готов (5.18); DHT (K4-2) и Worker (K4-3) — дальше |
