...
Первый рабочий шаг сделан в раунде 51: приглашение реально поднимает счётчик
`qualified_direct_count_v1`, до этого его не писал никто. Конверт атрибуции
`APUREF1` идёт транспортом 1:1, зачисляет фактического отправителя пакета,
идемпотентен по узлу и разбирается до авто-создания контакта
(`android-app/app/src/main/java/com/vladimir/messenger/data/referral/`).

Решение владельца от 2026-08-29: на время теста засчитывается ЛЮБОЙ, кто пришёл
по ссылке и начал чат; правило «Invitee создал новую identity» добавляется
вторым шагом в `ReferralCreditPolicy.decide`. Пункты ниже остаются незакрытыми:
подписанного receipt пока нет, D7-подтверждения для высоких ступеней нет,
антифрод против device-farm нет.
