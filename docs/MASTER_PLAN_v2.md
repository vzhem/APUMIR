...
| K5 | Кастодия сообщений у соседей (`file_custody*.rs` → движок и мост): доставка офлайн без брокера | 🔧 K5-1 код готов (5.21): копия сообщения у двух соседей по прямому каналу; файловая кастодия (`file_custody*.rs`) — K5-2 |
