...
| 5.17 Ядро, этап K3: куски личных файлов бинарными APUF-кадрами по прямому QUIC (`send_file_chunk` + событие `file_chunk_received`, приоритет данных, FCAP-рукопожатие) | ✅ | v11.70.28 (выпущен 2026-09-18, Latest): карта — `AI_HANDOFF.md` 0v, план — `CORE_ROADMAP.md` K3. На телефонах не проверено. Первый тег v11.70.27 не собрался (Kotlin: suspend-вызов `transport.send` из обычной `sendFcap`), лечение вошло в v11.70.28 |
...
| K3 | Куски файла бинарным QUIC-потоком (`send_file_chunk` + событие `FileChunkReceived`), не текстом через кадры/брокер | ✅ v11.70.28 (5.17) |
