...
| 5.5 Резервная копия и восстановление (просьба владельца 2026-09-12) | ⬜ | `IdentityBackup` восстанавливает только личность (ключ, имя, ник); чаты, контакты, группы, привязки шифрования и файлы после переустановки теряются. Нужна кнопка в настройках «Сделать резервную копию» → зашифрованный файл на телефоне (Room `messenger_database`, `p2p_prefs`, `binding_*`, вложения), и восстановление из него при входе. Подробности — `AI_HANDOFF.md`, пункт 0k |

### ПРИОРИТЕТ 6 — звонки и голосовые чаты

| Фаза | Статус | Основание |
|---|---|---|
| 6.1 Voice calls | 🟡 | `data/call/*` с v11.30: LAN-сокет 42109 проверен в одной Wi-Fi; через мобильную сеть с v11.70.14 мост через брокер + ADPCM (`CallBrokerLink`) — best-effort, на телефонах не проверено; UDP/STUN — только по «да» владельца (`CALLS_BOOTSTRAP.md` §8.4) |
| 6.2 Video calls | ⬜ | — |
