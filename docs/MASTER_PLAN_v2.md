...
| 5.14 Ядро, этап K1: постоянные QUIC-соединения (один endpoint, пул, keep-alive, STUN с порта 7777, ответ по входящему соединению) | 🟡 | v11.70.24 (в работе 2026-09-15): `rust-core/src/network/direct_transport.rs`, `SharedUdpSocket` в `quic_client.rs`, `handle_direct_frame` в `core.rs`; мост не менялся. Статус выпуска — `START_HERE.md` §3. На телефонах не проверено. Подробности — `AI_HANDOFF.md`, пункт 0t |
