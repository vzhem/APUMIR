...
| 5.5 Резервная копия и восстановление (просьба владельца 2026-09-12) | 🟡 | с v11.70.16: Настройки → Безопасность → «Резервная копия» → зашифрованный файл `.apubak` (`data/backup/*`: Room `messenger_database`, prefs, секреты подписи и обмена файлами, аватар, превью, по желанию полученные файлы); восстановление там же и с первого экрана «Войти». Владелец подтвердил 2026-09-13: работает. Не восстанавливаются передачи в пути и очередь ядра. Подробности — `AI_HANDOFF.md`, пункт 0k |
| 5.6 Автообновление файла копии (просьба владельца 2026-09-13) | 🟡 | с v11.70.17: карточка «Обновлять копию автоматически» (день/неделя/месяц) под сделанной копией; `BackupSchedule` + `AutoBackupWorker` (WorkManager, постоянный SAF-доступ к файлу, пароль завёрнут Keystore, двухфазная запись). На телефонах не проверено. Подробности — `AI_HANDOFF.md`, пункт 0l |
| 5.7 Рой, этап 8: два хранителя на файл и инвентарь недостающего | 🟡 | с v11.70.17: `FileCustodySender.TARGET_HOLDERS`=2, `CUSTODY_WANT` (тип 8), `ACK_RELEASE`; карта — `CHANNEL_SWARM_DESIGN.md` §9.4 «Этап 8». На телефонах не проверено. Подробности — `AI_HANDOFF.md`, пункт 0m |
| 5.8 Контакт-призрак «Contact APUCALL1» после звонков (скриншот владельца 2026-09-13) | 🟡 | с v11.70.18: приёмник отбрасывает сообщения с отправителем не-узлом (`util/NodeIds.kt`), уборка призраков при старте, голосовой фолбэк звонка в конверте, страж `pk_` в `core.rs`. На телефонах не проверено. Подробности — `AI_HANDOFF.md`, пункт 0n |
| 5.9 Рой, этап 9: файлы в группах раздают участники | 🟡 | с v11.70.19 (выпущен 2026-09-15, тег на `abbb5c4`): скрепка в группе, визитка `APUFILE1:` в сообщении, `fwant`/`fhave`, `GroupFileSwarm`; карта — `CHANNEL_SWARM_DESIGN.md` §9.4 «Этап 9». Владелец 2026-09-15 (v11.70.20): файл 35.8 МБ в группе на трёх телефонах получен — «пока всё работает». Подробности — `AI_HANDOFF.md`, пункт 0o |
| 5.10 Рой, этап 10: остатки без ядра (просьбы на диске, запасной сид, отказ от лишней копии, открытые сообщества, файл в посте канала) | 🟡 | с v11.70.20 (выпущен 2026-09-15, тег на `d3f61bc`): `GroupFileRequestStore`, `CANCEL_DECLINED`, `GroupFileCard` в ленте канала; карта — `CHANNEL_SWARM_DESIGN.md` §9.4 «Этап 10». Полосы от нескольких сидов — только с ядром. На телефонах не проверено (владелец проверяет всё после этого релиза). Подробности — `AI_HANDOFF.md`, пункт 0p |
| 5.11 Замечания с телефона после v11.70.20 + рой, этап 11 (`fnone`, уборка файлов при выходе из группы) | 🟡 | с v11.70.21 (выпущен 2026-09-15, тег на `843aabe`): заголовок темы в группе, закреп файла, «у меня нет» → следующий сид сразу, `GroupFileStore.deleteGroup`. Подробности — `AI_HANDOFF.md`, пункт 0q |
| 5.12 Системный жест «Назад» внутри темы → список тем (замечание владельца 2026-09-15) | 🟡 | с v11.70.22 (выпущен 2026-09-15, тег на `8c2dbc5`): `BackHandler` в `GroupChatScreen`. Владелец подтвердил после v11.70.21: заголовок темы и закреп файла работают. На телефоне не проверено. Подробности — `AI_HANDOFF.md`, пункт 0r |
| 5.13 Снятие заморозки ядра: CI перегенерирует uniffi-мост из `lib.udl` (просьба владельца 2026-09-15 «ядро самое лучшее») | 🟢 | v11.70.23 (выпущен 2026-09-15, тег на `ca29530`, прогон 35005340748): workflow скопирован владельцем (`ad70242`), мост перегенерирован, `core_build_info()` + строка «Ядро» в настройках, CI закоммитил мост и `.so` в main (`551f0d4`). На телефоне строка не проверена. План дальнейших этапов K1–K6 — `docs/CORE_ROADMAP.md`; код — `AI_HANDOFF.md`, пункт 0s |
| 5.14 Ядро, этап K1: постоянные QUIC-соединения (один endpoint, пул, keep-alive, STUN с порта 7777, ответ по входящему соединению) | ✅ | v11.70.24 (выпущен 2026-09-16): `rust-core/src/network/direct_transport.rs`, `SharedUdpSocket` в `quic_client.rs`, `handle_direct_frame` в `core.rs`; мост не менялся. Статус выпуска — `START_HERE.md` §3. На телефонах не проверено. Подробности — `AI_HANDOFF.md`, пункт 0t |
| 5.15 Ядро, этап K2: файл группы полосами от нескольких сидов (общий манифест `grp_`, `create_group_file_manifest`, `GroupFileSeeder`, `WANT` тип 9, `fwant#g1`) | ✅ | v11.70.25 (2026-09-17, предыдущий Latest): карта — `CHANNEL_SWARM_DESIGN.md` §9.4 «Этап K2», детали — `AI_HANDOFF.md`, пункт 0u. На телефонах не проверено |
| 5.16 Жест «Назад» на главном экране → вкладка «Все», а не выход из приложения (замечание владельца 2026-09-17) | ✅ | v11.70.26 (выпущен 2026-09-17, Latest; тег на `5eea3b7`, прогон 35214892864): `BackHandler(enabled = settledPage != 0)` в `ChatListScreen` - с вкладок «Группы», «Каналы», «Чаты» «Назад» возвращает на «Все», с «Все» закрывает приложение как обычно. На телефоне не проверено |
| 5.17 Ядро, этап K3: куски личных файлов бинарными APUF-кадрами по прямому QUIC (`send_file_chunk` + событие `file_chunk_received`, приоритет данных, FCAP-рукопожатие) | 🔧 код готов | 2026-09-17, ветка `arena/01a0af3e-apumir` (PR #6), ждёт явного разрешения владельца на релиз (следующий номер после v11.70.26). Карта — `AI_HANDOFF.md` 0v, план — `CORE_ROADMAP.md` K3. В CI не собрано, на телефонах не проверено |

| 5.18 Ядро, этап K4-1: presence «своим» и редкий маяк вместо объявления всем (меньше трафика в большой сети) | 🔧 код готов | 2026-09-17, ветка `arena/01a0b097-apumir`: модуль `presence_scope.rs` (маяк раз в 10 мин, «своим» лично раз в минуту по QUIC, порог 50 узлов), приём кадров `ppres`, `set_presence_audience` в мосте, CI-проверка компиляции ядра на PR (`scripts/ci/ci-core-check.yml` + установщик `scripts/install-ci-core-check.ps1`). В маленькой сети поведение прежнее. Карта — `AI_HANDOFF.md` 0w, план — `CORE_ROADMAP.md` K4-1. В CI не собрано, на телефонах не проверено |
| 5.19 Ядро, этап K4-2: поиск адреса по nodeId прямыми каналами (DHT наружу) | 🔧 код готов | 2026-09-17, ветка `arena/01a0b097-apumir`: модуль `address_lookup.rs` (кадры `dhtq`/`dhtr`, кольцо до трёх ближайших по XOR-расстоянию из `dht.rs`, кулдаун 5 минут, очередь ответов), вопросы и ответы в прямом канале, ответы отправляет поток presence, известные адреса не перетираются чужим ответом. Фолбэк — маяк K4-1 и mDNS. План — `CORE_ROADMAP.md` K4-2. В CI не собрано |
| 5.20 Ядро, этап K4-3: свой MQTT-брокер из настроек, строгий таймаут, фолбэк на публичные | 🔧 код готов | 2026-09-17, ветка `arena/01a0b097-apumir`: `multi_broker.rs` (`parse_broker_endpoint`, `probe_broker` с таймаутом 6 с, порядок «свой первым, публичные за ним»), выбор брокера в `mqtt_transport.rs` (не ответил никто — публичный по умолчанию), настройка `EngineConfig.own_broker` + мост `set_own_broker`, переменная `APU_MQTT_BROKER` для компьютера. Серверная часть (MQTT на Cloudflare Worker через WebSocket + Durable Object) — отдельным шагом. План — `CORE_ROADMAP.md` K4-3. В CI не собрано |
| 5.21 Ядро, этап K5-1: копия сообщения у двух соседей - доставка офлайн без брокера | 🔧 код готов | 2026-09-17, ветка `arena/01a0b097-apumir`: модуль `custody_relay.rs` (кадры `cust|offer/ack/deliver/drop`, конверт тот же `relay|…`), предложение копии «своим» соседям, отдача получателю потоком presence при появлении, `drop` автору после отдачи, ограничения 64 сообщения / 16 на получателя / 2 копии, защита от дублей; кастодия выключена по умолчанию, включение - `set_custody_enabled`. План — `CORE_ROADMAP.md` K5-1. В CI не собрано |
| 5.23 Ядро, этап K5-2: файловая кастодия в движке и мосте | 🔧 код готов | 2026-09-17, ветка `arena/01a0b097-apumir`: модуль `network::file_custody_relay` (кадры `fcust|offer|ack|deliver|drop`, `unit_id` из полей куска, очереди и пределы), подключение к движку (приёмник кадров, поток presence: ответы, выдача получателям, предложения соседям, чистка склада) и мост (`set_file_custody_enabled(enabled, db_path)`, `file_custody_usage_bytes()`, `offer_file_chunk_for_custody(...)`); склад `network::file_custody` с подписанными квитанциями открывается лениво и только с разрешения владельца. Приложение пока ходит своим путём (Kotlin `FileCustodySender`) |
| 5.22 Ядро, этап K6: тесты ядра блокируют проверку и релиз | 🔧 код готов | 2026-09-17, ветка `arena/01a0b097-apumir`: `cargo test --lib` входит в проверку на PR и становится блокирующим по маркеру `scripts/ci/tests-blocking` (включён после первого зелёного прогона 27b96c6); в копии релизного воркфлоу шаг `Core tests (K6)` вызывает `scripts/ci/release-tests.sh` до сборки APK, ставится скриптом `scripts/install-release-workflow.ps1`. Правила тестов дальше меняются обычным коммитом. Проверка YAML воркфлоу — `scripts/ci/check-workflow-yaml.py` |

Обзор плана ядра K0–K6 (единственный подробный текст — `docs/CORE_ROADMAP.md` §2; здесь только сводка со статусами):

| Этап ядра | Суть | Статус |
|---|---|---|
| K0 | Снятие заморозки: CI перегенерирует uniffi-мост из `lib.udl` | ✅ v11.70.23 (5.13) |
| K1 | Постоянные QUIC-соединения: один endpoint, пул, keep-alive, STUN с 7777, ответ по входящему соединению | ✅ v11.70.24 (5.14) |
| K2 | Файл группы полосами от нескольких сидов (общий ключ файла) | ✅ v11.70.25 (5.15) |
| K3 | Куски файла бинарным QUIC-потоком (`send_file_chunk` + событие `FileChunkReceived`), не текстом через кадры/брокер | 🔧 код готов (5.17), ждёт релиза |
| K4 | Presence и поиск узлов без линейного роста: presence только «своим», DHT наружу, свой rendezvous/relay на Cloudflare Worker (таймаут + фолбэк обязательны) | 🔧 К4-1, К4-2 и кодовая часть К4-3 готовы (5.18–5.20), ждут прогона CI; серверная часть К4-3 (MQTT на Worker) — дальше |
| K5 | Кастодия у соседей: доставка офлайн без брокера | 🔧 код готов (5.21, 5.23): K5-1 — копия сообщения у двух соседей по прямому каналу; K5-2 — файловая кастодия в движке и мосте (кадры `fcust|…`, склад `file_custody` с подписанными квитанциями, `set_file_custody_enabled`) |
| K6 | `cargo test --lib` для `rust-core` в CI при каждом теге; падение тестов = нет релиза | 🔧 код готов (5.22): тесты блокируют проверку на PR (маркер `scripts/ci/tests-blocking`), шаг `Core tests (K6)` в копии релизного воркфлоу ставится владельцем одним скриптом |
