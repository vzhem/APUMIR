...
## Статус 2026-08-20 (поздний вечер) — F3 Windows gate PASS + File-HELLO handshake

Гейт на новой машине (`C:\APU-M8`) закрыт: `testDebugUnitTest` 88/88, `assembleDebug` PASS
(история исправлений — доп.308). Найден и закрыт deadlock «первого файла» (пин ключа получателя
был возможен только из входящего offer): добавлен File-HELLO — крошечное подписанное
durable-сообщение, которым телефоны автоматически обмениваются ключами до первого файла
(доп.309). Телефонный file-acceptance — следующий шаг.

## Статус 2026-08-20 (вечер) — F3: файлы поехали по durable-пути (source, build gate закрыт 2026-08-20 позже)
