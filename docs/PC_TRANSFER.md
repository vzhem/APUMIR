...
Копия живёт на белой флешке с меткой APU_BACKUP (буква может меняться —
скрипт ищет по метке). После каждого большого обновления агент предлагает:
«вставьте белую флешку и выполните одну команду». Эта одна команда:

```
powershell -NoProfile -ExecutionPolicy Bypass -File C:\APU-M8\scripts\backup-to-flash.ps1
```

Скрипт сам найдёт флешку по метке, обновит папку APU-BACKUP\APUMIR (зеркало
без build-папок), перепишет apumir-full.bundle и проверит его целостность.
Если флешка не вставлена — напишет напоминание и ничего не тронет.
