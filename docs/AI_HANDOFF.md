...
0j. **Звонки через мобильную сеть — мост через брокер (2026-09-12, в
   v11.70.14, на телефонах НЕ проверено).** Жалоба владельца: в одной Wi-Fi
   ...
   всё ещё выше троттлинга публичных брокеров — это best-effort; настоящее
   решение — UDP + STUN hole punching или свой relay (Cloudflare Worker /
   WebSocket), оба ждут отдельного «да» владельца. Проверка на телефонах:
   два телефона в разных сетях, лог `CallManager: media channel: broker link`,
   статус на экране, разговор ≥ 1 мин. Старые сборки (< v11.70.14) `cap`/`ac`/
   мост не знают — с ними всё как раньше (PCM, текстовый фолбэк).
