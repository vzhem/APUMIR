...
- Репо: `github.com/vzhem/APUMIR`.Arena-сессия даёт СВОЮ рабочую ветку
  `arena/...` — работай только в ней, пуш только в неё.
...
9. Файлы в группах на 1000+ участников. Текущая передача файла строго 1:1:
   свой key envelope и binding на получателя, поэтому «отправить файл в группу»
   сегодня означало бы 1000 отдельных передач и 1000 конвертов ключей. Решение
   (владелец передал выбор 2026-08-27): не фанаут, а кастодия — отправитель
   кладёт файл один раз, в группу уходит короткий пакет «файл доступен» с
   SHA-256 (влезает в конверт 16 КиБ), участники забирают байты у ближайшего
   хранителя или у отправителя по LAN. Для этого нужно сначала подключить
   mesh-кастодию из пункта 4 (`rust-core/src/file_custody.rs` написан, но не
   подключён к движку/FFI/Android). До этого вложения в группах не включаем.
