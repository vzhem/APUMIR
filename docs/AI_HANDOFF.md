...
0w. **K4-1 - ядро: presence «своим» и редкий маяк вместо объявления всем
   (код готов 2026-09-17 на ветке `arena/01a0b097-apumir`; РЕЛИЗА НЕТ -
   ждёт явного разрешения владельца). Что где:**
   ...
   - Ждут своей очереди: K4-2 (DHT `dht.rs` наружу для поиска адреса по
     nodeId) и K4-3 (свой rendezvous/relay на Cloudflare Worker третьим
     брокером, таймаут + фолбэк). План - `CORE_ROADMAP.md`, этап K4.
