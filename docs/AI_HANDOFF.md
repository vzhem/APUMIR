...
0b. **Релиз v11.70.4 - в работе (2026-09-09).** Ветка = `main` (PATCH без
   force). Первый тег на `a6946b1` упал в `compileReleaseKotlin`: в
   `SwarmSettings.isMetered` стояла несуществующая константа
   `RESTRICT_BACKGROUND_STATUS_ENFORCED` (правильно `_ENABLED`). Релиз при
   этом не создался (job «Create Release» skipped), поэтому тег удалён и
   поставлен заново на исправленный коммит - версия и заметки остались
   v11.70.4. Дальше по проверенному порядку: дождаться CI → `gh release edit
   v11.70.4 --title "APU v11.70.4" --prerelease=false --latest --notes-file
   docs/RELEASE_NOTES_v11.70.4.md` → sha256 из `digest` ассета → строка в
   `START_HERE.md`. Если упадёт снова - чинить и перетегировать v11.70.5.
