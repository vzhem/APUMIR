...
0z. **WSS-мост MQTT: сообщения в «жёсткой» мобильной сети (белый список
   хостов).** Вторая жалоба оттуда же, где объявление об обновлении:
   TCP-1883 к брокерам сеть не пускает, потому MQTT молчал и сообщения не
   отправлялись. Решение — запасной путь через наш relay-домен (TLS-443 к
   workers.dev белый список пропускает), обычные сети НЕ изменены.

   - **Worker (владельцу передеплоить!):** в `tools/worker/p2p_relay_worker.js`
     добавлены: класс `MqttBridge` (Durable Object: WebSocket телефона <->
     TLS-сокет к брокеру; перебор broker.emqx.io:8883 -> hivemq:8883 ->
     mosquitto:8883, `connect` из `cloudflare:sockets`, `secureTransport on`),
     и маршрутизация: любой запрос с заголовком `Upgrade: websocket` уходит в
     мост (путь не важен, rumqttc строит `wss://.../mqtt`). 101-ответ несёт
     `Sec-WebSocket-Protocol: mqtt` — rumqttc без эха субпротокола рвёт
     соединение (validate_response_headers). Порядок деплоя: вставить файл
     целиком в Cloudflare, затем Settings -> Bindings -> Add -> Durable
     Object Namespace: имя `MQTT_BRIDGE`, класс `MqttBridge` (при первом
     деплое Cloudflare предложит миграцию new class). Проверка в консоли
     браузера: `new WebSocket("wss://p2p-relay.1985vzhem.workers.dev/mqtt",
     "mqtt")` — должен открыться (onopen, protocol "mqtt"); 1011 — мост не
     достучался до брокеров (или привязка не добавлена).

   - **Ядро (mqtt_transport.rs):** в `connect_with_shared_state`, когда НИ
     ОДИН TCP-кандидат не ответил на пробу (жёсткая сеть), подключение идёт
     на `wss://p2p-relay.1985vzhem.workers.dev/mqtt` (константа
     WSS_BRIDGE_URL): rumqttc 0.25.1 feature `websocket`, `Transport::Wss`
     c `TlsConfiguration::Rustls` (webpki-roots + aws-lc-rs). Адрес при WSS —
     ПОЛНЫЙ URL (split_url сам достаёт домен/порт). Вторичный брокер
     (mqtt-dual-broker) — тоже через мост. Новый deps: `webpki-roots = 0.26`.
     На обычной сети ветка не выполняется: TCP-проба отвечает раньше.

   - E2E-шифрование не менялось (MQTT — только сигнальный транспорт);
     free-лимиты Cloudflare DO достаточны для сигнального трафика.

   - **Деплой выполнен (2026-09-19):** дашборд не умеет создавать namespace
     класса (нужна миграция), поэтому владелец задеплоил скриптом
     `tools/worker/deploy-mqtt-bridge.ps1` (REST API, токен «Edit Cloudflare
     Workers»): класс MqttBridge + привязка MQTT_BRIDGE созданы, KV
     (APU_VAULT/REGISTRY/RELAY) сохранены, worker.js 634 строки. Проверка
     рукопожатия из браузера владельца пройдена (2026-09-19): консоль на
     обычном сайте, `new WebSocket("wss://p2p-relay.../mqtt","mqtt")` -> `OPEN
     mqtt` (на внутренних страницах chrome:// тест не проходил из-за их CSP —
     не ошибка моста). Полевой тест v11.74.3 показал ещё хитрее: сеть ОТВЕЧАЕТ
     ConnAck (рукопожатие проходит), но дальше режет данные - «ConnAck 54 с
     назад, ошибка Network timeout». Поэтому в v11.74.4 добавлен
     авто-фейловер: 4 ошибки poll подряд на прямом TCP -> флаг
     MQTT_PREFER_WSS + перезапуск сессии сразу через wss-мост; 6 ошибок
     подряд на мосте или полчаса его стабильной работы -> ревизия обратно на
     прямой TCP. Карточка «Сеть сообщений» стала тапабельной: диалог с
     расшифровкой и «Скопировать» (владелец не мог «зайти внутрь»). Внимание на будущие правки воркера: деплой дашбордом
     теряет namespace класса — обновлять тем же скриптом (правится только
     ссылка $WorkerUrl на новый тег).

0x-2. **Продолжение WSS-задачи: почему мост мог не работать на телефоне и
   что добавлено (2026-09-19, после релиза v11.74.2).** Полевой тест
   владельца: сообщения в жёсткой сети всё ещё не ходили; при этом браузерная
   проверка wss-моста (OPEN mqtt) прошла. Виновник найден в коде: проба
   брокера (`probe_broker` в rust-core/src/network/multi_broker.rs) проверяла
   только установку TCP-соединения (6 с). В сетях, где порт «приоткрыт», а
   полезный трафик режется (белые списки, DPI), проба объявляла мёртвый адрес
   живым - ядро садилось на него и никогда не добиралось до WSS-моста.
   Теперь проба делает НАСТОЯЩЕЕ MQTT-рукопожатие (минимальный CONNECT,
   клиент probe1, ждём CONNACK с кодом 0); заглушки и чёрные дыры отсеиваются,
   и в жёсткой сети выбирается wss-мост.

   Добавлено к этому же релизу (v11.74.3, просьба владельца):
   - диагностика MQTT в настройках: `mqtt_link_summary()` (mqtt_transport.rs)
     приклеивается к строке `core_build_info()`; SettingsViewModel отделяет её
     в карточку «Сеть сообщений» (режим tcp/wss, давность ConnAck, последняя
     ошибка). Новых FFI-функций нет - только хвост строки, поэтому lib.udl и
     p2p_core.kt не менялись;
   - самосев: `ApkSeeder.ensureSelfSeeding()` - приложение само раздаёт свой
     текущий APK (applicationInfo.sourceDir, копия в seedRoot как обычно),
     пока человек не останавливал раздачу (флаг autoseed.v1, ставится
     `unmark()`, снимается ручными пометками); вызов - initFromDisk (в фоне,
     init под mutex!) и 5-минутный блок pump;
   - adoptDownloadedApk: скачанный файл, равный текущей версии, пересаживает
     раздачу со старой версии на себя (раньше случай comparison == 0 молча
     пропускался - владелец обновлялся из приложения, а телефон продолжал
     раздавать 11.74.1);
   - карточки-призраки: dropStaleReady() прячет «Установить vX» при версии
     не старее v; scanReceivedApks не показывает принятые APK старее/равные
     текущей («11.70.22 получен»).

0x-3. **«Адрес в конверте»: ответы после смены сети адресатом (2026-09-19,
   v11.74.5).** Сценарий владельца: Аня сменила сеть (Wi-Fi -> мобильный
   интернет), её сообщение дошло, а ответы к ней висели «часиками». Причина:
   адресат строил прямой путь по её СТАРОМУ адресу (азбука не успела
   обновиться: presence раз в минуту + STUN раз в ~55 с, у Ани сессия
   брокера после смены сети поднималась не сразу). По сути владельц
   предлагал: «пусть пришлёт свой адрес прямо в сообщении». Так и сделано:

   - ОТПРАВКА: `P2PCore::send_message`, когда прямой QUIC не вышел и
     сообщение уходит в офлайн-релей, ядро следом кладёт в топик получателя
     (`p2pm2/msg/<id>`, через MqttOutboundCommand::MeshRelay) кадр
     `ppres|node_id|name|addr|relay|version|ts`
     (presence_scope::direct_presence_payload с текущим public_addr из
     STUN). message_id помечается префиксом ppres-.
   - ПРИЁМ: обработчик `p2pm2/msg/` в run_mqtt_transport узнаёт кадр по
     префиксу ppres| ДО разбора обычных сообщений: проверки те же, что у
     QUIC-личного presence (версия + tolerance, возраст <=
     PRESENCE_MAX_AGE_MS, node_id != свой, pk_-страж внутри парсера), адрес
     пишется в peer_addrs + AddressBook («адресата_и public»). Старые
     сборки кадр молча пропускают (не 4 поля с pk_ первым) - N-1 не задет.
   - Дубликаты сообщения не страшны: incoming insert IGNORE по message_id
     (ChatRepository.addIncomingMessage -> insertMessageIgnore).
   - Прямой путь с новым адресом может упереться в CGNAT; тогда помогает
     «тёплый» NAT: STUN-запрос раз в ~55 c держит отображение, и входящее
     QUIC доходит. Офлайн-релей остаётся запасным путём доставки.

0x-4. **Расщепление роя по брокерам (критично, 2026-09-19, v11.74.6).**
   Тест владельца на двух v11.74.5: «Аня отправляет - доходит, ей отправляют
   - висит». Причина НЕ в адресах: прямой клиент (multi_broker BROKERS) идёт
   на hivemq; телефон на WSS-мосте - на emqx (MQTT_UPSTREAMS моста начинался
   с emqx). Брокеры не федерируются: сообщения p2pm2/# не пересекают границу.
   Аня после «жёсткой» сети оставалась на мосте (флаг PREFER_WSS жив до
   ревизии), собеседник на прямом - два отдельных роя. Фиксы:
   - worker: MQTT_UPSTREAMS теперь hivemq:8883 первым (как в ядре) -
     ВЛАДЕЛЬЦУ ПЕРЕДЕПЛОИТЬ воркер скриптом (ссылка в скрипте обновлена на
     v11.74.6); до передеплоя мост по-прежнему уводит на emqx;
   - ядро: на стабильном мосте раз в минуту CONNACK-проба прямого брокера;
     ответил - снятие PREFER_WSS + перезапуск сессии на прямой путь
     (MQTT_DIRECT_PROBE_EVERY_MS=60s); ревизия стабильности 30 -> 5 мин;
   - карточка «Раздаю обновление»: maybeAutoReseed больше не стирает самосев
     при временно отсутствующей строке (показывает preparing);
     ensureSelfSeeding достраивает потерянную строку раздачи своей версии;
     startSeeding пропускает копирование, если источник = копия (самосев).

   «Адрес в конверте» (0x-3) покрывает и группы: GroupDelivery -
   PerMemberFanoutDelivery поверх того же 1:1 send_message, фолбэк каждого
   члена группы несёт его ppres. Идеи владельцу на будущее (стабильность):
   дублирование публикации в 2 брокера для мелких групп, свой брокер
   на workers.dev (MQTT over WebSocket штатно, DO-мост уже есть), ping/pong
   уровня приложения для раннего обнаружения «тихой резки» трафика,
   хранение азбуки адресов в Kotlin-слое с экспортом/импортом.

0x-5. **Наш MQTT-брокер в worker'е (2026-09-19, v11.74.7).** Идея
   владельца «полноценный маленький сервер» доведена до ключевого элемента:
   DO MQTT_BRIDGE (все телефоны и так сходились в один его инстанс) больше
   НЕ пайпит на hivemq/emqx - он САМ брокер: минимальный MQTT 3.1.1
   (CONNECT/CONNACK, SUBSCRIBE/SUBACK + granted 0, PUBLISH с фанатом всем
   прочим подписчикам, PUBACK на входящие QoS1, retain-карта для presence,
   LastWill, PINGREQ/PINGRESP, wildcards + и #), фреймы буферизуются
   (1 ws-сообщение может нести и кусок пакета, и несколько пакетов).
   Ограничения: 400 клиентов, payload <= 256КБ, retained <= 512 тем.
   Ядро: наш брокер - ПЕРВЫЙ выбор (skip TCP-проб), статусы MQTT_OWN_FAILED_
   AT_MS/CHOSEN_OWN: не ответил ConnAck за 20 с (MQTT_START_CONNACK_TIMEOUT)
   -> пометка отказа, час идём на публичных, потом снова пробуем наш.
   Побочный фикс: висящий старт сессии (недостижимый брокер) больше не
   подвешивает connect_ready_mqtt_session - таймаут возвращает Err, движок
   перезапускает сессию по своему циклу. ВЫПОЛНЕНО 2026-09-19:
   владелец переподдеплоил worker (дважды: сначала старый скрипт v11.74.7,
   потом свежий) — брокер ЖИВОЙ, владелец проверил из консоли браузера
   (WebSocket /mqtt, subprotocol mqtt → OPEN).
   ВАЖНО: retained-состояние DO в памяти - после рестарта DO presence
   восстанавливается за минуту (перепубликация), это норма.

0x-6. **Раздел «Сервер» в настройках + резервные копии азбуки
   адресов (2026-09-19, v11.74.8).** Владелец: «резервные копии азбуки
   адресов это самое главное» и «в настройках сделать вкладку "сервер" и
   там уже сделать раздел все по серверу» (веб-панель не понадобилась).
   Реализовано:
   - worker: POST /addrbook/put, GET /addrbook/get?shelf=… (полка
     «addrbook|<node_id>», до 200 КБ, KV APU_VAULT; байты непрозрачны для
     сервера). Вступает в силу ПОСЛЕ переподдеплоя воркера владельцем
     (см. 0x-5 — тот же скрипт деплоя, ссылка с тега v11.74.8);
   - Kotlin data/backup/AddressBookBackup.kt: шифрование AES/GCM,
     ключ = SHA-256(приватный ключ узла + «:apu-addrbook-v1»), конверт
     «APUADDBK1|base64(iv+sealed)»; автокопия из CoreServerService
     (первая через 3 мин, дальше каждый час, шлёт только при изменении
     файла); восстановление на свежей установке ДО RustBridge.initialize
     (ядро читает apu_peer_addresses.json один раз при старте);
     restoreNowForce — только по явной кнопке (заменяет файл, вступит
     в силу после перезапуска приложения);
   - настройки: раздел «Сервер» = адрес и отклик сервера (замер /health
     через BotApi.pingHealth), «Сообщения (рой)» (переехала из «О
     приложении»), «Резервная копия адресов» с диалогом «Создать
     копию»/«Восстановить».
   Статус: worker с /addrbook ЗАДЕПЛОЕН владельцем 2026-09-19, копия
   азбуки создана с телефона успешно («прям сразу сохранилось»).
   Грабли: (1) decrypt возвращает БАЙТЫ — перед JSONObject/writeText нужен
   String(bytes, UTF_8) (E: «Unresolved reference 'has'» в CI на этом и
   погорело, исправлено до релиза). (2) Текст ошибок Kotlin CI читается из
   АННОТАЦИЙ check-run: gh api repos/…/commits/<sha>/check-runs →
   /check-runs/<id>/annotations — работает, когда логи недоступны.
   (3) При КАЖДОМ изменении worker'а поднимать ссылку тега в
   deploy-mqtt-bridge.ps1: в v11.74.8 забыли — скрипт скачал и задеплоил
   worker от v11.74.7 (без /addrbook), предупреждение о длине не спасло.
   Теперь скрипт прерывает деплой, если в скачанном файле нет addrbook/put;
   при следующих релизах менять $WorkerUrl и содержательную проверку.
