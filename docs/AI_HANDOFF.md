...
0. **Свежее, 26 сентября (раунды 162-173) - статус.**
   СДЕЛАНО и выпущено (Latest = v11.74.70): свои стикеры (.zip и по
   одному) в панели всех чатов; роевой каталог «Из сети» с мгновенным
   объявлением; стикеры парят без фонов; анимация .webm покадрово
   (WebP у vibesticker статичен - владельцу качать паки в WEBM);
   меню/удаление/увеличение с непрерывной анимацией; ANR при пропавшей
   сети починен (ресивер без перезапусков на main-потоке); закрепы:
   группы (переход), личка и каналы (бар + булавка). Все телефоны
   обновить. ПК-патчи: 40..70 -> 71 (владелец, self-check ok) и
   71 -> 72 (владелец, self-check ok) - лестница полная, долг закрыт
   26.09. Порядок для следующих релизов - cd C:\APU-M8, ветка
   arena/01a0b575-apumir (на main нет tools/ci!), git fetch --tags,
   затем цикл - см. START_HERE раздел 7.
   АУДИТ-2 (кандидаты, ждут «добро»): markOnline файлов - на КАЖДЫЙ
   пульс без гейта (гейт по knownPeers, как у лёгких пульсов);
   кэш сжатого аватара для whois-ответов (сейчас компресс на каждый
   ответ). Опрос событий и уведомление - уже в норме (5 с/100 мс,
   дедуп текста).
   ДАЛЬШЕ: (1) закрепы в «Избранном» - миграция БД, отдельный раунд;
   (2) очеловечить «Сообщения сети» (MQTT) - ждёт решения владельца;
   (3) ReferralInviteLink -> apumir.app - не наш домен, ссылки не вести.
0a. **Сентябрь (раунды 79-96) - остатки.** v11.69.3 проверена владельцем:
...
0x-7. **Копии азбуки на телефонах роя (2026-09-19, v11.74.9).**
   Решение владельца: «зачем ограничиваться только на своих… нужно
   сохранять копии рандомно, так больше шансов им выжить», количество —
   «хотя бы на 5». Реализовано поверх облачной копии (0x-6):
   - жеребьёвка хранителей УСТОЙЧИВАЯ: кандидаты (контакты ∪ проверенные
     ∪ привилегированные ∪ узлы азбуки) сортируются по sha256(мой|кандидат),
     первые 5 - хранители. Случайно кто, стабильно надолго; рейтинг
     СОЗНАТЕЛЬНО не используется (владелец: у «лучших» общие слабые места);
   - протокол поверх обычной 1:1 доставки (как APUREAD1): APUBK1|store|<владелец>|<конверт>
     / ack / ask / give; конверт - тот же APUADDBK1 (E2E от владельца,
     хранитель байтов не видит);
   - досылка хранителю не чаще раза в 12 ч (кнопка - force всем пятерым);
     ack свежий 48 ч -> строка «На телефонах роя: N из 5» в диалоге;
   - восстановление: «Восстановить» пробует сервер, при пустом - ask роя
     (до 40 знакомых, ожидание 20 с), берёт первый конверт, открывшийся
     ключом владельца;
   - хранитель: filesDir/apu_swarm_backups/<владелец>.bk, максимум 20
     владельцев, TTL 60 дней, размер конверта <= 120 КБ (потолок брокера
     256 КБ после запечатывания);
   - у хранителя ответ шлёт routeIncoming в CoreServerService (до
     автоконтактов); отправка только через RustBridge.sendMessage с
     chatId=получатель (паттерн HeartRepository).
   Предел (честно): свежая установка при мёртвом сервере и без знакомых
   восстановиться не с кого - тогда QR от доверенного (он отдаёт и копию
   азбуки теперь автоматически, ask по контакту).
   ПОЛЕ 2026-09-19 (v11.74.9 у владельца): «создать копию» = «роздана на
   4», счётчик confirm-ов 0-1. Это НОРМА при смешанных версиях: счётчик
   считают только ACK от хранителей НА v11.74.9; хранитель на старой
   версии копию получить не может осмысленно - APUBK1|store не узнаёт,
   конверт падает ему в чат длинной строкой (и заводится автоконтакт).
   Порядок при будущем изменении протокола APUBK1: СНАЧАЛА обновить все
   телефоны, ПОТОМ жать «Создать копию»; мусорные строки на старых
   телефона удалять руками. Возможно на будущее: служебные конверты
   нового поколения слать только узлам с подтверждённой новой версией
   (ack несёт версию).

0x-8. **Финиш чата 2026-09-19: состояние и что СРОЧНО для владельца.**
   Линия v11.74.5→v11.74.9 (все выпущены, Latest = v11.74.9, apk
   40 223 910 Б, sha256 348be680…36112, тег на d0c4b2f):
   v11.74.5 «адрес в конверте» (ppres в offline-relay) → v11.74.6 фикс
   расщепления роя + стабилизация самосева APK → v11.74.7 СВОЙ
   MQTT-брокер в worker'е → v11.74.8 облачная копия азбуки + раздел
   «Сервер» в настройках → v11.74.9 копии азбуки на 5 случайных
   телефонах роя. Воркер ЗАДЕПЛОЕН владельцем (брокер жив: `OPEN mqtt`;
   /addrbook работает: копия создана и восстановлена с телефона).
   Долги владельца (не отменены): (1) удалить ОБА светившихся в чате
   Cloudflare-токена; (2) обновить ВСЕ телефоны до v11.74.9 (хранители
   на старых версиях не подтверждают копии и складывают конверт в чат
   длинной строкой); (3) после обновления — снова «Создать копию»,
   счётчик «На телефонах роя» должен стать 4-5 из 5 (сейчас 0-1).
   После v11.74.9 (не выпущено отдельно, следующий тег v11.74.10):
   значок непрочитанных комментариев НА КАЖДОМ ПОСТЕ канала (просьба
   владельца). Механика уже была: GroupDao.incrementTopicUnread на каждом
   чужом сообщении темы (GroupRepository ~2642), сброс - GroupChatViewModel
   markRead при чтении комментариев (комментарии канала открываются
   GroupChatScreen по topic-маршруту), значок канала = сумма по темам.
   Добавлено: ChannelPost.unreadComments = topic.unreadCount (ChannelViewModel
   observe), UnreadBadge у кнопки «Комментарии» (ChannelScreen, стиль из
   GroupChatScreen). Нюанс: новый пост показывает «1» (сам пост), пока не
   откроют комментарии. Сетевого трафика ноль.
   v11.74.11: адрес сервера скрыт из настроек (просьба владельца:
   «убрать конфиденциальную информацию 1985vzhem») - строка называется
   ...
   v11.74.14: GIF в темах групп/каналов (просьба владельца). GIF ходит
   УЖЕ существующим файловым роем (скрепка -> stage -> APUFILE1-визитка),
   автозагрузка мелких была (shouldAutoFetch). Добавлена АНИМАЦИЯ:
   (1) зависимость coil-gif 2.7.0; (2) MessengerApplication implements
   coil.ImageLoaderFactory - общий ImageLoader c GifDecoder (+ImageDecoder
   Decoder на API 28+), без него AsyncImage даёт первый кадр;
   (3) GroupFileMarker.isGif (MIME или .gif по имени); (4) GroupFileCard:
   ветка AsyncImage(File) анимацией, previewFile и по имени GIF;
   (5) PhotoViewer.ZoomablePhoto: File-GIF -> AsyncImage, рамка зума по
   bounds-only decode (decodeBounds), масштаб/клэмп работают. Старые
   версии видят «📎 имя.gif». Отправка по-прежнему под рангом и правами
   (SEND_STICKERS «Стикеры и GIF»).
   v11.74.14 (каталог GIF выбран владельцем дополнительно к анимации):
   кнопка «GIF» в поле ввода темы/канала -> GifCatalogDialog (поиск +
   сетка 3хN, AsyncImage превью, «Ещё» по курсору). Поиск идёт через НАШ
   worker /gif/search -> Tenor v2 (featured при пустом q), ключ в
   binding TENOR_KEY (деплой-скрипт читает необязательный tenor-key.txt
   рядом с собой; нет ключа - честная «не настроен»); выбранная гифка
   качается телефоном с CDN Tenor (BotApi.downloadGif, кап 25 МБ) и
   прикладывается как ФАЙЛ: GroupFileSwarm.stageGifBytes (тот же ранг/
   sha256/store.put/визитка mediaType image/gif). Внешняя точка только
   каталог; содержимое - наш рой. Не забыть: owner должен (1) получить
   ключ Tenor, (2) положить tenor-key.txt, (3) повторить деплой
   (WorkerUrl в скрипте уже v11.74.14).
   Дополнение (2026-09-21): у владельца НЕ грузится Library в Google
   Cloud (телефон и ПК, стабильно) - ключ Tenor не получить. Воркер
   доработан: /gif/search умеет GIPHY_KEY (Giphy v1, trending/search,
   «ещё» = offset) ПОВЕРХ Tenor: сервер берёт GIPHY_KEY, если задан,
   иначе TENOR_KEY; формат ответа тот же (приложение не менялось).
   Деплой-скрипт: giphy-key.txt -> binding GIPHY_KEY (рядом с
   tenor-key.txt); WorkerUrl теперь закреплён за SHA c662c4172c795a93170
   ...
   поколения - только узлам с подтверждённой версией); тесты ядра (K6)
   в релизный воркфлоу. Процедура релиза не изменилась: пробник
   v0.0.0-sandbox-checkN (СЛЕДУЮЩИЙ N = 18) на ТОЧНОМ релизном
   коммите -> success -> удалить пробные тег+релиз -> ask владельца ->
   тег -> promote заметками -> verify /releases/latest. Текст ошибок
   CI: аннотации check-run'ов (грабля в 0x-6). Песочница регулярно
   пересобирается: `git fetch origin arena/01a0b575-apumir &&
   git reset --hard FETCH_HEAD` и проверять свежесть файлов.
