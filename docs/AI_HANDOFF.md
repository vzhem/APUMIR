...
## Текущее состояние (на 2026-08-26)

Релиз **v11.18.0** выпущен и является Latest:
https://github.com/vzhem/APUMIR/releases/tag/v11.18.0 (собран GitHub Actions
по тегу, workflow `.github/workflows/build-release.yml`; версия приложения
берётся из имени тега).
...
6. Раздел ГРУППЫ — код написан (схема БД v8, темы, заявки, приглашения с QR,
   поиск участников, публичная/частная, статистика, разрешения, закрепы), но
   **ни разу не скомпилирован**: в песочнице не было ни cargo, ни JDK/Gradle.
   Первый шаг — прогнать `scripts/groups-build-gate.ps1` на Windows.
