...
0b. **Релиз v11.70.4 - ждёт токена GitHub.** В ветке готовы три коммита после
   `18c3331` (рой кусок 1 `5c23050`+`2ada518`, репост в два шага `4f56f2a`
   + заметки `docs/RELEASE_NOTES_v11.70.4.md`), но push и `gh` отвечают
   «Bad credentials». Порядок, когда токен оживёт: `git push origin
   arena/01a07cda-apumir` → `gh api -X PATCH repos/vzhem/APUMIR/git/refs/heads/main
   -f sha=<HEAD> -F force=false` → `git tag -a v11.70.4 <HEAD> -m "APU v11.70.4"`
   → `git push origin refs/tags/v11.70.4` → через ~13 мин `gh release edit
   v11.70.4 --title "APU v11.70.4" --prerelease=false --latest --notes-file
   docs/RELEASE_NOTES_v11.70.4.md` → sha256 из `digest` ассета → правка
   `START_HERE.md`. Владелец велел: «сначала выпусти релиз для всех, потом
   сделаешь остальное». Если CI упадёт (код роя и репоста не компилировался) -
   чинить и перетегировать v11.70.5.
...
0a. **Раздача роем (главная задача после v11.70.4).** Владелец 2026-09-09:
   ...
   приоритеты и пределы). Состояние кода: см. `AI_COLLABORATION_NOTES.md`,
   последняя запись. Ничего из роя на телефонах не проверено.
