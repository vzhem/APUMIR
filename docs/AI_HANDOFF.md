...
0b. **Релиз v11.70.4 выпущен 2026-09-09** (Latest, prerelease снят, заметки
   из `docs/RELEASE_NOTES_v11.70.4.md`, тег на `bc50494`, sha256 см.
   `START_HERE.md`). Первый тег на `a6946b1` упал в `compileReleaseKotlin`
   из-за `RESTRICT_BACKGROUND_STATUS_ENFORCED` (правильно `_ENABLED`); релиз
   тогда не создался, поэтому тег переставлен без смены номера. Ждём проверки
   владельца на телефоне: репост с текстом и ссылкой, фото на весь экран,
   «Раздача» в настройках.
