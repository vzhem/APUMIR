...
0x. **Рой APK: обновление APU раздаётся телефонами роем, без сервера
   ...
   (манифест/конверты/куски/ACK) НЕ трогалась. Провод — ТРИ НОВЫХ вида
   ...
   (пришли мне), `upnone|ver|sha256` (нет, спроси другого).
   Что где:
   ...
   - `ui/screens/settings/*` — раздел «Обновления»: моя раздача («получили:
     N» + «Остановить»), предложение соседа (только новое), приём с
     прогрессом и «Остановить», «Версия vX готова» → «Установить»,
     «Раздать полученный APK» (сценарий: владелец переслал APK с ПК —
     отметить файл и ввести/подтвердить версию), «Отметить APK как
     обновление» (SAF-выбор файла, версия в диалоге).
