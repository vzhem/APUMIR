...
0b. **Релиз v11.70.5 выпущен 2026-09-09 (Latest, тег `c535800`): репост
   одним сообщением + «Без ограничений».** Проверка владельцем v11.70.4: фото на весь экран - ок,
   «Раздача» - ок, репост в два шага - «сложно». В v11.70.5 репост =
   ACTION_SEND одной картинки (фото или сетка `PhotoCollage`) с подписью
   (текст + ссылка, ≤ 1000 знаков). Ждём проверки на телефоне: в Telegram и
   WhatsApp должно прийти ОДНО сообщение с картинкой и подписью. Если Telegram
   откроет свой редактор фото перед отправкой - это его поведение при шаринге
   в конкретный чат, подпись там уже подставлена.
