...
0x-5. **Наш MQTT-брокер в worker'е (2026-09-19, v11.74.7).** Идея
   ...
   перезапускает сессию по своему циклу. ДО 20.09 владелец ОБЯЗАН
   переподдеплоить worker скриптом (ссылка уже на v11.74.7): пока не
   переподдеплоил, старый мост пайпит на hivemq и наш брокер не жив.
...
0x-6. **Раздел «Сервер» в настройках + резервные копии азбуки
   ...
   - настройки: раздел «Сервер» = адрес и отклик сервера (замер /health
     ...
     копию»/«Восстановить».
   Грабли: decrypt возвращает БАЙТЫ — перед JSONObject/writeText нужен
   String(bytes, UTF_8) (E: «Unresolved reference 'has'» в CI на этом и
   погорело, исправлено до релиза). Текст ошибок Kotlin CI читается из
