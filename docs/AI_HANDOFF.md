...
0e. **v11.70.9 - короткие ссылки (просьба владельца 2026-09-10: «чтобы в
   ссылках не было видно информацию… сделай короткие ссылки»).** Код в
   `data/link/*`, карта - `START_HERE.md` «Ссылка пересылки поста».
   **Worker должен опубликовать владелец** (файл целиком отправлен в чат;
   шаг 3 `docs/IDENTITY_VAULT_SETUP.md`); до этого приложение делится
   длинными ссылками. Что проверить на телефонах после публикации worker'а:
