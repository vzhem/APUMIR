...
0c. **v11.70.7 - рой, этап 2 (первая волна, эстафета, полосы, `peers`);
   выпуск в работе 2026-09-09 вечером** - состояние проверять по
   `gh api repos/vzhem/APUMIR/releases/latest --jq .tag_name`. Карта кода -
...
0a. **Раздача роем (главная задача после v11.70.4).** Владелец 2026-09-09:
   ...
   `peers` при N > 100) - v11.70.7; дальше этап 3 (подпись в ядре, куски по
   QUIC). Ничего из роя на телефонах не проверено.
