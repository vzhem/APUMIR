...
0. **Свежее, сентябрь (раунды 79-96).** v11.69.3 проверена владельцем:
   ...
   - **владелец ещё не опубликовал новый код worker'а** - `/i` (страница
     «Открыть в APU / Установить») и `/.well-known/assetlinks.json` отвечают
     404. Файл готов: `tools/worker/p2p_relay_worker.js`; порядок - сначала
     worker, потом APK с `autoVerify` (Android проверяет файл при установке);
   - не проверено на телефонах: ссылка открывается в APU без вопроса «чем
     открыть» (нужен опубликованный assetlinks), вставка всего пересланного
     сообщения в «Войти по ссылке», нажатие на нашу ссылку внутри чата APU;
   - публичное описание v11.69.3 - заглушка CI; текст готов в
     `docs/RELEASE_NOTES_v11.69.3.md`; релизы v11.69.1/v11.69.2 пустые
     (без APK) - удалять только с разрешения владельца;
   - ключ подписи `android-app/app/p2p-release.jks` и пароль к нему лежат в
     ПУБЛИЧНОМ репозитории (`build.gradle.kts`). Любой может подписать APK «как
     APU», и телефон примет его как обновление. Вынести в GitHub Secrets -
     отдельная задача, требует решения владельца (смена ключа = телефоны не
     обновятся поверх, придётся переустанавливать).
