...
0. **Свежее, сентябрь (раунды 79-96).** v11.69.3 проверена владельцем:
   ...
   - **ключ подписи раскрыт**: `android-app/app/p2p-release.jks` и пароль
     ...
     предупреждает). С 2026-09-15 копия снова новее живого файла - см.
     грабли «CI падает до сборки» ниже и `START_HERE.md` §3. Телефоны
...
0o. **v11.70.19 - рой, этап 9: файлы в группах (код в `main` = `cd40803`,
   тег стоит, релиза НЕТ: CI падает до сборки на `setup-android` - что
   делать владельцу и боту, `START_HERE.md` §3; статус выпуска - там же).** Просьба владельца «доделывай систему
...
- **CI падает до сборки: `Failed to find package 'tools'` (2026-09-15,
  ...
  копирует владелец. Журнал упавшего прогона читать через ссылку из
