...
0. **Свежее, сентябрь (раунды 79-95).** Не проверено на телефонах:
   кликабельная https-ссылка пересылки поста и страница `/i` на worker'е -
   **владелец ещё не опубликовал новый код worker'а**, до публикации ссылка
   даст 404. Файл готов: `tools/worker/p2p_relay_worker.js`.
