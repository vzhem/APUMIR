...
> ⚠️ **Не публиковать через `git tag ... && git push --tags`.** Это запустит
> `.github/workflows/build-release.yml`, который пересоберёт APK из
> закоммиченных `.so` (без нового Rust-ядра) и создаст битый релиз. Публикуйте
> через `gh release create` или веб-интерфейс (ниже) — они не триггерят workflow.
> Готовый фикс workflow (ручной режим + draft-релиз) лежит в
> `docs/release-workflow.manual.yml` — применить его можно вручную.
