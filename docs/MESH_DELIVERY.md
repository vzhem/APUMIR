...
Манифест, привязывающий ключ к телу и к паре узлов (одинаковый у обеих сторон):

```
apu-msg-v1|<отправитель>|<получатель>|<sha256 шифротекста в hex>
```

Подменить тело или переадресовать конверт другому получателю нельзя: ядро
сверяет манифест и отпечаток привязки получателя при вскрытии.
