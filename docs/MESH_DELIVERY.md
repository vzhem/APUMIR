...
- **Совместимость разных версий.** Rolling update не должен разделять mesh: поддерживаемые N и
  N-1 обязаны понимать совместимый wire version/capabilities, доставлять relay/receipt/cleanup в
  обоих направлениях и безопасно отклонять неизвестный обязательный формат без enqueue/UI/ACK.
  Каждый protocol change проходит mixed-version 3-phone matrix, включая relay-узел другой версии.
...
- **Надёжность receipt:** r1 reconnect и r2 unique retained receipt+cleanup проверены на
  ...
  M3(d); then persistence + final acceptance. r4.3 status-only EMQX observer source complete behind
...
## Чистка списка узлов (v11.56.0)

Брокер хранит последнее объявление каждого узла бессрочно, поэтому при
подключении телефон получал «я онлайн» от всех установок, которые когда-либо
существовали. Отсюда 46 «подключённых устройств» при пяти реальных телефонах.

Что теперь отсекает мусор:

1. В presence добавлены два поля — версия формата и отметка времени. Старые
   сборки читают первые четыре поля и не ломаются.
2. Объявление без даты не принимается вовсе: проверить его свежесть нечем,
   а именно такие записи и копились у брокера. В список попадают только те,
   кто прислал дату.
3. Объявление старше `PRESENCE_MAX_AGE_MS` (сутки) игнорируется.
4. Сведения от узлов, отставших на `PRESENCE_VERSION_TOLERANCE` (3) версий и
   больше, не принимаются — ни presence, ни список знакомых через gossip.
5. Собственное объявление, вернувшееся из брокера, ставит отметку «я был в
   сети с таким-то адресом»; если она не обновляется, это видно в логе.
6. Узлы, о которых известно только с чужих слов, живут `HEARSAY_STALE_SECS`
   (90 секунд) вместо `PEER_STALE_SECS` (200) и удаляются автоматически.

Удаление всегда автоматическое: срок вышел — запись пропала. Узел вернётся в
список сам, как только снова объявится в сети.

## Рейтинг узлов (v11.59.0)

Сеть держится на самих телефонах, поэтому важно знать, кому отдавать данные
первым. Оценка 0..100 считается локально, только из собственных наблюдений —
объявить себя хорошим нельзя, можно лишь вести себя хорошо.

Из чего складывается:

| Признак | Вес | Почему |
|---|---|---|
| Доступность (был на месте / пропустил круг) | 35 | недоступный узел бесполезен, каким бы быстрым ни был |
| Прямая достижимость (внешний адрес) | 25 | к такому узлу дотянутся все, а не только соседи по Wi-Fi |
| Скорость прямой отдачи | 20 | на ней экономится время передачи файлов |
| Доля удачных доставок | 15 | обещал и донёс |
| Свежесть последней встречи | 5 | вчерашний узел не должен обгонять сегодняшнего |

Ещё не измеренный узел получает нейтральные 0.5 по надёжности: иначе новые
собеседники навсегда оставались бы в конце очереди.

Где применяется:

- `PerMemberFanoutDelivery` обходит получателей в порядке рейтинга — при обрыве
  связи на середине веера данные успеют уйти хотя бы надёжным.
- Настройки → «Узлы сети» (бывшее «Подключено пиров») показывают список с
  оценкой, долей времени в сети, скоростью, доставкой и прямой связью.

Наблюдения собираются в `PeerRatingStore`: обнаружение узла и пропуск круга —
из `CoreServerService`, скорость и результат отдачи — из `FileTransferSender`.

## Шифрование переписки (v11.60.0)

До этой версии текст личных сообщений уходил через ретранслятор открытыми
байтами: прямой QUIC защищён TLS 1.3, но путь через публичный брокер
(hivemq/emqx) — это чужой сервер, который видел переписку. Файлы, звонки и
служебные пакеты шифровались, а сама переписка — нет.

Теперь каждое сообщение уходит запечатанным.

### Схема

Гибридная, ровно как у файлов:

1. На каждое сообщение — свежий случайный ключ 32 байта.
2. Тело шифруется AES-GCM (256 бит, тег 128 бит, случайный вектор 12 байт).
3. Ключ запечатывается X25519-конвертом ядра `create_file_key_envelope`
   для одного получателя, с подписью Ed25519 отправителя.

Свой ключ на сообщение: вскрытие одного не раскрывает остальные.

### Формат на проводе

```
APUSEAL1|<конверт ключа base64>|<вектор base64>|<шифротекст base64>
```

Манифест, привязывающий ключ к телу (одинаковый у обеих сторон):

```
apu-msg-v1|<sha256 шифротекста в hex>
```

Имён узлов в манифесте НЕТ намеренно. Один и тот же узел может называться
32- или 64-символьным видом своего ключа (`is_legacy_routing_node_id`
допускает оба), и на разных путях доставки приходит разный вид. Пока имена
входили в манифест, сообщения через ретранслятор не открывались, хотя прямая
связь по Wi-Fi работала. Подлинность сторон это не ослабляет: она проверяется
подписанными привязками внутри самого конверта, а привязка ключа к телу
сохраняется отпечатком шифротекста.

### Ключи

Отдельной инфраструктуры ключей не заводилось. Используются те же подписанные
привязки X25519, что и для файлов: они расходятся пакетом HELLO по всем
контактам и закрепляются по принципу «доверяем первому увиденному»
(`FileExchangePeerStore`). Смена ключа у известного контакта отвергается.

- `MessageSealer` — запечатывание, вскрытие и кэш ключей.
- `SealedWire` — разбор формата на проводе.
- Запечатывание врезано в единственную точку отправки `RustBridge.sendMessage`
  (и `sendDirectPayload`), вскрытие — в единственную точку приёма
  `CoreServerService.handleEvent`, до всех разборщиков. Поэтому шифруется всё,
  что ходит этим транспортом: личные сообщения, группы, каналы, реакции,
  реферальные и файловые служебные пакеты.

### Исключения

- **HELLO** (`apu-file-hello1|`) идёт незапечатанным: он несёт сам открытый
  ключ, и шифровать его нечем, пока ключа нет. Секрета в нём не содержится.
- Пока ключ собеседника неизвестен, сообщение уходит открытым и пишется
  предупреждение в журнал. Это возможно только до первого обмена HELLO.
  Кэш ключей разогревается на старте (`warmSealingKeys`), поэтому у уже
  работающих установок открытых сообщений после обновления не будет.

### Совместимость

Формат новый и со старыми версиями не согласуется: узел без этой версии
получит нераспознанный конверт. Обновляются все телефоны сразу — это
осознанное решение владельца, переходный режим не делался.
