...
2. Объявление старше `PRESENCE_MAX_AGE_MS` (10 минут) игнорируется.
3. Сведения от узлов, отставших на `PRESENCE_VERSION_TOLERANCE` (3) версий и
   больше, не принимаются — ни presence, ни список знакомых через gossip.
4. Собственное объявление, вернувшееся из брокера, ставит отметку «я был в
   сети с таким-то адресом»; если она не обновляется, это видно в логе.
5. Узлы, о которых известно только с чужих слов, живут `HEARSAY_STALE_SECS`
