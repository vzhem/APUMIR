...
## Профиль, карточка собеседника, смахивание и залипшее имя

- Нижняя панель: «Избранное» → «Профиль» (`ApuTab.Profile`). Избранное вернулось
  в меню «⋮» главного экрана.
- `Screen.Profile` — отдельный маршрут, открывает тот же `SettingsScreen` с
  `showProfile = true`. Вкладки сверху убраны: в настройках теперь список, где
  «Профиль» — первый пункт.
- `ui/components/PeerProfileSheet.kt` — карточка собеседника по тапу на имя в
  шапке переписки: аватар, имя, @никнейм, «в сети», узел с копированием,
  кнопки «Позвонить» и «Переименовать». Ничего не грузит по сети.
- `GroupChatScreen`: внутри темы смахивание вправо = «Назад» к списку тем.
  Порог `SWIPE_BACK_THRESHOLD_PX = 140f` в пикселях (доля ширины на планшете
  требовала бы слишком длинного жеста). Гасим только движение вправо.
- **Причина «буквыцифр» вместо имени:** `isPlaceholderName` не считала
  заглушкой кусок node_id, поэтому настоящее имя из presence его не подменяло.
  Добавлена `looksTechnical` (префикс `pk_` либо длинное слово из латиницы с
  цифрами) — сторожит `PlaceholderNameTest`. Плюс подстраховка в
  `CoreServerService`: имя в таблице чатов чинится по контакту.

## Общий жест «смахнуть назад» и бегунки

`ui/components/SwipeBack.kt` — `Modifier.swipeBack(enabled, onBack)`, порог
`SWIPE_BACK_THRESHOLD_PX = 140f`. Гасим только движение вправо, поэтому
горизонтальные списки внутри экрана продолжают работать. Подключён: переписка,
группа (внутри темы — к списку тем, иначе к группам), список групп, контакты,
избранное, настройки, профиль, ранги, лента канала.

Бегунок добавлен в настройки, профиль и ранги.

## Запрос «представься» (whois)

Имя приходило только роевой рассылкой раз в шесть часов, поэтому собеседник
подолгу висел набором букв и цифр. Добавлен адресный пакет `APUGRP1|who|<id>`:

- отправляется, когда и в контакте, и в объявлении имя техническое;
- один запрос на узел не чаще десяти минут, а как только имя пришло — вообще
  не отправляется;
- в ответ узел шлёт своё @имя и аватар именно спросившему.

Если имя уже нормальное, ничего не запрашивается — обычная редкая рассылка.

## Приглашение в группу по QR при личной встрече

«Пригласить» теперь спрашивает: показать QR или отправить ссылку.

- QR создаёт НОВОЕ приглашение с `requestApproval = false` — сканирование
  пускает в группу сразу, без подтверждения владельца. Оправдано тем, что люди
  стоят рядом и показывают друг другу телефоны.
- Обычная ссылка «поделиться» остаётся строгой: её могут переслать кому угодно,
  и в частной группе она по-прежнему требует одобрения.
- Приёмная сторона ничего не меняла: сканер QR на главном экране уже узнаёт
  групповые ссылки и ведёт в раздел «Группы» со входом.
- Одинаково работает и в разделе «Группы», и в меню пузыря на главной.

- **2026-09-05 (доп.79) - шаг тестов ядра в гейте требует ЕЩЁ И C-компилятора, не только cargo:**
  в `groups-build-gate.ps1` добавлен step 0b (`cargo test --lib`), потому что ядро собирает только
  CI и ошибка Rust всплывала через ~13 минут на релизном сервере. Первая версия проверяла лишь
  наличие `cargo`. На машине владельца cargo стоит, а MSVC `cl.exe` нет, и `ring`/`aws-lc-sys`
  (обе компилируют C) упали с `ToolNotFound: cl.exe`, exit 101 - гейт остановился до сборки, хотя
  с кодом всё было в порядке. Исправлено: шаг пропускается, если нет cargo ИЛИ C-компилятора.
  **MSVC ради этого шага ставить не нужно** - по доп.201/202 host-сборки C на этой машине уже
  приводили к массовым C errors, `0xc0000006` и BSOD, а Visual Studio стояла на выведенном из
  эксплуатации диске D:. Настоящая проверка ядра - сборка в CI на каждом релизе.

- **2026-09-07 (раунды 79-95) - шифрование переписки, личность, каналы; релизы
  v11.59.0 - v11.69.0.** Сделано и подтверждено владельцем на телефонах:
  сквозное шифрование всей переписки (`data/security/MessageSealer` +
  `SealedWire`, гибрид AES-GCM на сообщение + X25519-конверт ядра
  `create_file_key_envelope`; Double Ratchet НЕ подошёл - нужен живой
  handshake, а ретранслятор доставляет не по порядку); синие галочки
  «прочитано» (`data/receipt/`); сердечки профиля (`data/heart/`); просмотры
  постов (`data/channel/`); защита личности никнеймом и паролем
  (`IdentityVault` + `IdentityBackup`, сундук на worker'е владельца);
  регистрация с никнеймом и паролем плюс вкладка входа; каналы - комментарии
  обычным чатом, посты с фото, пересылка постов.
  Схема БД 15 -> 17 (`profile_hearts`, `post_views`).

  **Дефекты, найденные владельцем на телефонах, и их причины** (все уже
  исправлены, но класс ошибок стоит помнить): половинчатая пара ключей при
  регистрации -> ядро выдавало НОВЫЙ адрес на следующем старте, QR указывал на
  мёртвый узел; MQTT-приёмник ждал 5 полей в теле вместо 4 -> терялись
  сообщения и обмен ключами; ACK принимался из чужих тем -> вторая галочка не
  появлялась; `APULAN1` шифровался -> прямой канал не поднимался, связь шла в
  одну сторону; HELLO рассылался только по чатам -> контакт из QR не получал
  ключ; вход в чат из «Контактов» передавал contactId вместо chatId -> пустой
  чат-двойник и вечные часики. Подробности - в `docs/AI_HANDOFF.md`, раздел
  «Типичные грабли».

  **Производительность.** Синхронная отправка ACK держала очередь входящих до
  10 секунд (разовое подключение к двум брокерам) - отсюда «Приложение не
  отвечает» каждые несколько секунд и расход батареи. ACK переведён на
  постоянное соединение и в фон, бюджет разового пути ужат до 3 с, сверка
  ключей - раз в 5 минут вместо 20 секунд.

  **Инфраструктура.** В гейт добавлены шаги `0b` (тесты ядра, пропускаются без
  cargo ИЛИ без C-компилятора) и `0c` (предупреждение, что собранное ядро
  старше исходников). Владелец развернул в своём worker'е `/vault/put` и
  `/vault/get`; код и инструкция - `tools/worker/p2p_relay_worker.js`,
  `docs/IDENTITY_VAULT_SETUP.md`. В `main` осталась НЕ проверенная на телефонах
  правка: https-ссылка пересылки поста и страница `/i` (её ещё нужно
  опубликовать в worker'е).

- **2026-09-07 (раунд 96) - ревизия v11.69.3: ссылка пересылки и отступ.**
  Владелец выпустил v11.69.3 (тег на `91916fb`, Latest, APK 36 961 166 байт,
  sha256 `891d2f3b…4084a`) и подтвердил на телефонах: «плюсик не мешает,
  ссылки активные». Проверка по коду, тегам, CI и живому worker'у нашла:
  (1) worker с `/i` не опубликован - ссылка без APU даёт `{"error":"not found"}`;
  (2) intent-filter стоял с `autoVerify="false"`, а assetlinks не было - на
  Android 12+ https-ссылка уходит в браузер, «перехват» не гарантирован;
  (3) регресс: `LINK_PATTERN` не знал https-форму, вставка всего пересланного
  текста в «Войти по ссылке» отвергалась; (4) четыре теста на ссылку поста
  использовали slug `abcd1234` вне алфавита - не могли пройти, никто не гонял;
  (5) релизы v11.69.1/v11.69.2 пустые (бот создал их раньше CI), описание
  v11.69.3 - заглушка CI с потерянными буквами А/У/Р.
  Сделано в ветке: `WEB_HOST`/`OFFICIAL_HOST` в `GroupInviteLinks`, https-форма
  в `LINK_PATTERN`, тесты на настоящем slug + два новых; worker: маршрут
  `/.well-known/assetlinks.json` с отпечатком релизного ключа
  (`F8:43:CB:E7…A5:F7`, снят с `p2p-release.jks`), экранирование параметров на
  странице `/i`; манифест: `autoVerify="true"`; `MessageBubble`: наша ссылка из
  чата APU открывается в APU (`setPackage`); workflow: исправлен шаблон
  описания; `promote-release.ps1 -NotesFile`; `RELEASE_NOTES_v11.69.3.md`.
  ...
  Вечером того же дня владелец опубликовал worker (код прислан в чат целиком:
  в `C:\APU-M8` лежит `main`, ветки сессии там нет). Проверено с двух сторон:
  `/i?slug=…` - страница «Запись в APU» с кнопками «Открыть в APU» и
  «Установить APU», `/.well-known/assetlinks.json` - JSON с
  `com.vladimir.messenger` и отпечатком `F8:43:CB:E7…A5:F7`, `/vault/get` жив.
  Дальше: гейт на Windows и релиз v11.70.0 из ветки (с разрешения владельца),
  затем ротация ключа отдельным заходом.

- **2026-09-08 (раунд 97) - v11.70.0: первый выпуск отменён, main догнан.**
  Владелец прогнал `sync-main.ps1 -WorkBranch arena/01a07cda-apumir` - FATAL
  «local branch does not exist» (на ПК ветки сессии нет), `main` остался на
  `b7f7971` (5 сентября). Гейт зелёный, `make-release.ps1 -Version v11.70.0`
  поставил тег на `b7f7971` - на 19 коммитов **старше** v11.69.3 (нет
  шифрования переписки, галочек, сердечек, каналов; схема БД 15 против 17 ->
  `fallbackToDestructiveMigration` стёр бы базу). Владелец попросил «переведи
  галочки, чтобы у всех появилось обновление» - НЕ переведены; релиз и тег
  v11.70.0 удалены (`gh release delete --cleanup-tag`), `main` переведён на
  кончик ветки fast-forward'ом из песочницы. Исправлены скрипты: `sync-main`
  берёт кончик из `FETCH_HEAD`, `make-release` проверяет, что HEAD содержит
  предыдущий тег и `origin/main`. Описание релиза -
  `docs/RELEASE_NOTES_v11.70.0.md`. Дальше: тег v11.70.0 заново на кончике
  `main`, CI, `promote-release.ps1 -NotesFile`.
  **Итог:** v11.70.0 пересобран на кончике `main` (`3a88e67`), Latest;
  владелец: «обновление у всех появилось… по ссылке тоже всё переходит по
  этапам». Замечание: первое окно по ссылке на пост спрашивало «Войти в
  группу?». Сделано `data/group/InvitePrompt.kt`: слова окна по признакам
  ссылки (`p=` -> «Открыть пост?», `c=1` -> «Подписаться на канал?», иначе
  группа; заявку обещаем только при `a=1`), те же правила для итогового
  окна в `GroupsViewModel.joinByLink`. Тест `InvitePromptTest`. На телефоне
  не проверено - ждёт следующего релиза.
  Второе замечание (скриншот раздела): вкладка «Группы» содержит и каналы.
  Переименовано в «Сообщества» (панель + заголовок экрана + две подсказки),
  без разделения на две вкладки - так решили раньше и владелец это
  повторил. Слова для каналов приведены к «подписке» во всех окнах выхода/
  удаления и в кнопке каталога. Ширина подписи проверена по метрикам Roboto
  Medium 11sp: «Сообщества» 64.7dp против «Настройки» 56.1dp - на 412dp
  экране умещается без многоточия (кнопке достаётся ~77dp).
  **Выпуск v11.70.1** (владелец: «Выпускай релиз для всех»): тег на
  `79b129a` из песочницы после ручных проверок make-release (содержит
  v11.70.0 и origin/main, дерево чистое), CI 34187514702 зелёный, APK
  sha256 `510d5997…24cd0`, prerelease снят, draft->publish, `/releases/latest`
  = v11.70.1. Workflow всё ещё старый (заголовок «Release + …» поправлен
  вручную) - владельцу по-прежнему нужно скопировать
  `scripts/ci/build-release.yml` в `.github/workflows/`.
  Проверка v11.70.1 владельцем (скриншоты): (1) после «Открыть пост» и
  «Назад» человек возвращался в «Сообщества», где снова всплывало «Вход по
  ссылке / Вы подписаны на канал» - `LaunchedEffect(joinLink)` срабатывал
  заново, потому что параметр остаётся в маршруте; (2) «Настройки» стало
  «Настрой…» - моя ошибка: считал ширину под 412dp, экран владельца 375dp.
  Сделано: `joinHandled` (rememberSaveable), под пост кладётся лента канала
  (`navigate(Channel)` + `navigate(GroupChat topic)` в одном кадре), панель
  делит ширину поровну и подбирает единый кегль по самой длинной подписи.
- **2026-09-08 (раунд 99) - посты канала: несколько фото, правка, досылка
  старых постов.** Владелец (после ANR-правки `f9e633d`): «в пост нужно
  больше фотографий», «чтобы можно было редактировать пост», «кто пришёл на
  канал позже - должен видеть старые посты и оставлять комментарии»; релиз
  - один на всё, после проверки. Уточнил через вопросы: до 6 фото, каждое
  отдельным пакетом, галерея в ленте, качество прежнее; править может автор
  и владелец; досылать последние 20 постов.
  **Почему куски, а не один пакет.** Единственный путь между телефонами вне
  одной сети - MQTT-брокер (`rust-core/src/network/mqtt_transport.rs`,
  rumqttc 0.25). У rumqttc `max_outgoing_packet_size` = 10 240 байт по
  умолчанию и нигде не переопределён; публичные брокеры (hivemq/emqx) тоже
  режут крупные PUBLISH - это уже проверялось файлами (`20d9c5b`: фрагменты
  12 КБ не приходили, стало 4 КиБ). Текст поста по пути кодируется трижды:
  GroupWire base64url -> AES-GCM seal base64 -> `relay|…|base64` -> при 7000
  символах фото relay-пакет ~18.5 КБ, то есть пост с фото ходил ТОЛЬКО по
  QUIC в одной сети. Поднять лимит в Rust недостаточно (брокер чужой), HTTP-
  почтового ящика у воркера нет. Поэтому `InlineImage.MAX_PART_B64_CHARS =
  3200` (relay-пакет ~9.5 КБ): фото на 7000 символов = 3 куска, пост из 6
  фото = 1 текст + 18 кусков. Куски - обычные сообщения темы с текстом
  `APUIMGP1:<фото>/<кусок>/<всего>:<base64>`; в тексте поста пометка
  `APUIMGS1:<n>`, чтобы лента знала, сколько ждать. Ни схема БД, ни
  доставка не менялись: `GroupRepository.sendMessage(…, photos)` кладёт
  куски в базу сразу (автор видит пост мгновенно), текст уходит первым, а
  веер кусков - в `backgroundScope` (IO), чтобы не держать экран и не
  обрываться при уходе с него. Приёмник (`handleIncoming Message`) куски
  хранит, но не двигает непрочитанное/превью/статистику;
  `GroupChatViewModel` и уведомления в `CoreServerService` их не показывают;
  `ChannelViewModel.observe` склеивает по `(тема, номер фото)` только ПОЛНЫЕ
  фото (`InlineImage.assemble`), недоехавшие показывает как «ещё N».
  **Правка.** `KIND_EDIT` = `edit|groupId|topicId|b64(messageId)|b64(текст)`,
  в конверте только слова (фото у получателей уже есть). Приём: от автора
  сообщения или владельца группы, иначе отброс; `InlineImage.replaceText`
  сохраняет служебные строки; если правится первое сообщение темы канала -
  тема переименовывается в первую строку (`postTitle`, 40 символов, как при
  создании). UI: карандаш в шапке карточки (`myId == authorId || владелец`),
  тот же `PostEditorDialog` с готовым текстом, фото показаны без правки.
  **Досылка.** Владелец при `admitMember` (через 1.5 с, чтобы карточка/темы/
  решение легли раньше) и по `KIND_POSTS_REQUEST` (`preq|groupId|limit[|
  have,…]`) шлёт последние 20 тем по времени создания: текст поста + его
  куски, каждый под исходным id, 8-е и 9-е поля конверта = автор и время
  (`buildMessage(authorId, sentAtMs)`); живые сообщения остаются 7-полевыми.
  Приёмник верит автору/времени только если отправитель - владелец; дубли
  отсекает `messageExists`; досланное не считается непрочитанным; если темы
  ещё нет - заводит её сам. Подписчик просит посты из `ChannelViewModel`
  (init + один раз после появления членства) и из `GroupChatViewModel`
  (по ссылке на пост человек минует ленту), не чаще раза в 30 с, со списком
  тем, чьи посты уже целые (`isPostComplete`). Владелец не повторяет узлу
  то, что слал за последние 8 с, держит паузу 40 мс между пакетами и потолок
  150 пакетов на веер (очередь ядра 256). Тесты: `GroupWireTest` (9 полей,
  7 полей у живых, edit, preq), `InlineImagePartsTest` (нарезка, склейка,
  порядок, неполные фото, правка). Компилятора в песочнице нет - проверено
  чтением; на телефонах НЕ проверено. Ограничения v1: фото при правке не
  меняются; телефон с прошлой версией не понимает 9-полевые конверты (старых
  постов не получит) и покажет куски как «буквы» в комментариях.
  **Выпуск v11.70.2** (владелец: «выпускай релиз для всех»). Первый push
  ветки сорвался - токен GitHub-подключения Arena протух («token is no
  longer valid»); к следующему ходу подключение ожило само, push прошёл.
  `main` догнан fast-forward'ом через API (`PATCH git/refs/heads/main`,
  `force=false`) до `9f6b414` - на ПК владельца ветки сессии нет,
  `sync-main.ps1` ему не с чего запускать. Тег на `9f6b414` (содержит
  v11.70.1 и origin/main, дерево чистое). Пуш тега породил ДВА прогона
  «Build Release APK» (в 15:32:05 и 15:32:09); отменить второй нельзя
  (403 у бота), оба собрались зелёными за ~13 мин, второй на шаге Create
  Release дописал в уже созданный релиз - ассет один, `app-release.apk`
  36 993 930 байт, sha256 `5da8db7561fd8241c54201e0f97e7e5d80cfeaef433fc9f
  be3f0e259265e8fb5`. Галочки: заголовок «APU v11.70.2», описание из
  `RELEASE_NOTES_v11.70.2.md`, prerelease снят, Latest; `/releases/latest`
...
- **2026-09-08 (раунд 100) — репост с фото, concurrency в CI, проект роя.**
  ...
  **Продолжение 2026-09-09.** Владелец выполнил команду: `git pull --ff-only`
  (`b7f7971` → `a022166`, первый раз GitHub не ответил - вторая попытка
  прошла), скопировал `scripts/ci/build-release.yml` в `.github/workflows`,
  коммит `2909051` в `main` («ci: re-sign step + concurrency for tag
  builds»). Проверено по дереву: установленный файл байт в байт равен
  `scripts/ci/build-release.yml` на `a022166`, то есть **шаг Re-sign есть,
  а `concurrency` - нет** (эта правка ушла в ветку позже, коммит
  `c1c0109`). Заголовок коммита владельца про concurrency поэтому
  преждевременный; повторное копирование донесёт её. Песочница снова
  откатилась (HEAD `91916fb`, объекты `d8e2709`/`16747c6` пропали, рабочее
  дерево с правками уцелело) - восстановлено `git reset a022166` (mixed) и
  перекоммичено одним коммитом `c1c0109`; токен GitHub ожил, push прошёл.
  `origin/main` (`2909051`) влит в ветку (`8691d47`, конфликтов нет: у
  владельца изменён только `.github/workflows/build-release.yml`), `main`
  догнан fast-forward'ом через API до `8691d47`. Оба указателя равны.
  **Релиз v11.70.3 (2026-09-09 утром).** Владелец сам докинул `concurrency`
  в живой workflow (`ae9076d`, «ci: concurrency group for tag builds») и
  сказал: «релиз для всех пока выпускай, потом доделаешь остальное». Ветка
  влита с `main` (fast-forward до `ae9076d`), написаны
  `docs/RELEASE_NOTES_v11.70.3.md` (`357576c`), `main` догнан через API,
  тег `v11.70.3` на `357576c`. Прогон CI один (34302894503, ~13 мин,
  success) - группа `concurrency` работает. Релиз отредактирован:
  «APU v11.70.3», Latest, prerelease снят, описание из файла заметок;
  `/releases/latest` отдаёт `v11.70.3`. Ассет `app-release.apk`
  36 993 802 байт, sha256
  `c6c239c8e4564ccd974e5df93f6c49f554fad576bae09b1fadb5b116ec1edb0f`
  (поле `digest` в API релиза; скачать ассет из песочницы не удалось -
  EOF). Это первая компиляция кода репоста с фото - собралось без правок.
  На телефонах не проверено. Песочница за этот раунд откатывалась ещё раз
  (HEAD `91916fb`); восстановлено `git reset --hard b19dc1a` +
  `git fetch --deepen=5 origin main` + merge.
  **Рой - новая постановка владельца.** «Всё, что роем можно сделать, -
  всю систему настраивать на ройную» и приоритеты: «сначала своим,
  проверенным, популярным, стабильным, потом всем подряд», с настраиваемым
  пределом на телефон. В `CHANNEL_SWARM_DESIGN.md` добавлен раздел 9
  (ярусы и пределы), в раздел 8 вписаны ответы по умолчанию, чтобы не
  ...
  **v11.70.4 у владельца (2026-09-09 12:51).** Фото на весь экран - «всё
  хорошо». «Раздача» есть, предложил добавить «без ограничений» (сделано:
  `SwarmMode.UNLIMITED`). Репост в два шага - «всё сложно», хотя при
  нажатиях доходит и текст, и фото; требование: одно нажатие, всё сразу, без
  лишней информации. Решение - одна картинка + подпись (см. START_HERE).
  Почему не «текст отдельным сообщением автоматически следом»: два
  `startActivity` подряд из одного нажатия невозможны - второй chooser
  перекрывает первый или теряется; Android не даёт узнать, куда человек
  отправил первый. Сетка - единственный способ уложиться в один ACTION_SEND.
  **v11.70.5 у владельца (2026-09-09).** «Репост лёгкий и с фото и с текстом
  и ссылкой. Всё переходит и работает.» Тема репоста закрыта. Дальше -
  «Продолжай дальше по плану этап роя».
  **Рой, этап 1 (v11.70.6).** Решения, которых нет в проекте §4–§7 или
  которые от него отступают:
  - в подпись манифеста добавлен `revision` (номер правки): без него после
    правки текста автором сиды отвергали бы исправленный текст по хэшу, а
    злоумышленник мог бы «откатить» правку, повторив старый манифест;
  - ключ подписанта едет прямо в `pman` (32 байта), а не только в привязке
    личности: полную привязку (`SignedIdentityBindingV1`, ~180 байт) в
    каждый манифест класть дорого, а её проверка через ядро ничего не даёт
    сверх того, что даёт подлинность отправителя пакета. Доверие ключу -
    через таблицу `post_signers`: сам узел прислал (ядро гарантирует
    отправителя) или владелец канала заверил. Пара «узел + заверитель» в
    ключе таблицы - чтобы владелец канала А не мог назначить узлу ключ для
    канала Б;
  - `preq` обслуживаем только тем, кто есть в моей таблице участников, и на
    открытом канале тоже (в §8.4 - «открытый: всем, кто предъявил манифест»):
    список участников владелец рассылает при каждом вступлении, так что
    честный новичок в таблице есть, а чужим трафик не тратим; смягчить можно
    одной строкой в ветке `PostsRequest`;
  - манифест только у поста (первое сообщение темы), комментарии в теме
    роем не ходят;
  - `eddsa` вместо ядра: FFI подписи произвольных байтов в uniffi нет, а
    биндинги в CI не пересобираются. `PostSigner` перед первой подписью
    сверяет выведенный ключ с ключом ядра; при расхождении подпись молча
    выключается (посты по-старому) и пишет ошибку в лог.
  Сделано без компилятора; проверка - CI. Тесты: `PostManifestTest`
  (вектора RFC 8032 §7.1 тест 1–3 с точными подписями), `SwarmBufferTest`.
  **v11.70.6 выпущен** (2026-09-09, прогон 34360928024 с первого раза,
  sha256 `8862111e…48e9`). Владелец: «Выпускай релиз для всех… и потом
  продолжай делать следующий этап» - выпуск и этап 2 без отдельного
  подтверждения. Токен GitHub снова отваливался и снова поднялся сам через
  несколько минут - владелец просил пробовать несколько раз, а не ждать.
  **Рой, этап 2 (v11.70.7).** Решения сверх проекта §3:
  - «умеет рой» = узел сам предъявил ключ подписи (`post_signers`,
    `nodeId = attestedBy`): для этого подписчик теперь шлёт `pkeys` со своим
    ключом владельцу и админам (в ответ на `pkeys` при вступлении и при
    первом принятом манифесте). Иначе автор не отличил бы старый телефон от
    нового и либо оставил бы старые без поста, либо слал бы всем;
  - деление на волну включается только когда умеющих рой больше K: канал
    из десятка человек ведёт себя ровно как v11.70.6 (нечего ломать при
    проверке на двух телефонах);
  - манифест от автора на маленьком канале дальше не идёт (все получили);
    на большом - идёт от волны и от каждого сида, случайным участникам
    (сид ключей соседей не знает; старый телефон отбросит один маленький
    пакет). Только для постов моложе 15 минут - старые, собранные через
    `preq`, эстафеты не заслуживают;
  - свежий манифест от самого автора → ждём куски до 90 с, а не просим у
    соседей, у которых их тоже ещё нет; иначе первая волна порождала бы
    лавину бесполезных `pwant`;
  - `m` полос = число известных сидов темы (кто прислал манифест или куски),
    2..4; недостающие места - лучшие соседи наугад;
  - сид отдаёт и неполный пост (что есть из полосы) - как торрент;
  - на открытом канале `preq`/`pwant`/`pkeys` принимаются и от тех, кого нет
    в таблице участников (на большом канале таблица заведомо неполная -
    `peers` даёт 30 из тысяч); закрытый - только известным, для чего
    соседям из выборки уходит `roster` из одной строки о новичке;
  - счётчик подписчиков на телефоне участника не опускается ниже числа от
    владельца (`bumpMemberCount`; `peers` задаёт его прямо) - иначе после
    `peers` карточка показывала бы «31 участник» у канала на 5 000;
  - `rreq` уходит только администраторам (они одни отвечают), а не всем.
  Пределы (§4): 3 полосы разом, 60 кусков/мин на отдачу; 3 поста разом на
  сбор. Схема БД не менялась. Сделано без компилятора; проверка - CI.
  **Фото в личке у отправителя обрезано (2026-09-09, в v11.70.7).** Раунд 43
  делал превью исходящей картинки аватарной функцией (квадрат из середины) -
  получатель видел файл целиком, отправитель квадрат. Заменено на
  `PhotoPreview.write` (пропорции сохраняются, ≤ 1280 по длинной стороне,
  EXIF-поворот), пузырь - `Fit` по пропорциям вместо `Crop`. Старые превью
  не пересчитываются (оригинала у нас нет).
  Токен GitHub 2026-09-09 17:15–17:35 не поднимался 20 минут (~60 попыток);
  владелец переподключил вручную, после этого всё прошло. Указание владельца:
  пробовать подключаться повторно «в разумных пределах», не зацикливаться.


- **2026-09-12 (раунд 113) — v11.70.13 выпущен; звонки через мобильную
  сеть: мост через брокер; задача «резервная копия».**
  v11.70.13 (рой, этап 7) выпущен по стандартной процедуре, Latest,
  sha256 ассета проверен. По просьбе владельца («паралельно проверь что со
  звонками») разобран путь звонка вне Wi-Fi - см. `AI_HANDOFF.md` 0j:
  голос через `sendMessageMqtt` открывал новое соединение с брокером на
  каждую публикацию и упирался в 5-секундный опрос событий на приёмнике;
  ни один голосовой кадр физически не мог доехать. Написан `CallBrokerLink`:
  собственный MQTT 3.1.1 на чистом Kotlin (без библиотек - в проекте их нет,
  без FFI - биндинги не пересобираются), одно соединение на звонок, тема
  вне `p2pm2/#`. Уроки: (1) `feedMachine` теперь двигает машину под замком -
  события приходят с трёх потоков (ядро, LAN-сокет, читатель моста), раньше
  два потока могли одновременно дёргать `mediaFrame`; (2) `cap` идёт
  ОТДЕЛЬНЫМ пакетом, а не полем в offer/accept - старые сборки разбирают их
  строго по числу полей; (3) счётчик nonce управляющих пакетов моста
  задаётся один раз на звонок и не сбрасывается при переоткрытии моста -
  ключ тот же, повтор nonce под GCM недопустим; (4) сторож темпа кадров
  считает кадры, а не пачки (`mediaFrames(now, n)`), иначе 4 кадра в одной
  публикации выглядели бы как один и звонок хоронили бы за «голодание».
  Владелец просил «запиши потом проверить»: после переустановки вход по
  логину/паролю не восстанавливает профиль - записано в `AI_HANDOFF.md` 0k
  и `MASTER_PLAN_v2.md` 5.5 с наброском «резервная копия в файл».
  Токен GitHub снова истёк (тринадцатый раз) ровно перед push - коммит
  локальный, повторные попытки по правилу владельца («в разумных пределах»).

- **2026-09-18 — v11.70.28 выпущен; первый тег v11.70.27 не собрался
  (Kotlin: suspend-вызов из обычной функции); логи CI из песочницы
  недоступны - причину нашли локальным компилятором.**
  Тег v11.70.27 (на `ee0b22b`, PR #8) упал в прогоне 35302706377 на шаге
  «Build Release APK» (Gradle exit 1, ~3м43с - умер в середине сборки;
  релиза и APK нет). Логи из песочницы агента не читаются: `gh run view
  --log-failed`, `gh api .../jobs/<id>/logs` и `gh run download` падают с
  EOF на presigned-ссылках `*.blob.core.windows.net`, а check-run отдаёт
  пустой output - эти попытки повторять бессмысленно.
  Причина найдена БЕЗ логов: в песочницу поставлены настоящие инструменты
  (`npm i kotlin-compiler` + `pip install --break-system-packages jdk4py`;
  JVM лежит внутри site-packages, компилятор запускается с
  `JAVACMD=<...>/java-runtime/bin/java`) и прогнаны исходники Android.
  Диагноз: `FileTransferReceiver.kt:346` - новая функция `sendFcap` (K3)
  объявлена обычной `fun`, а внутри `runCatching` зовёт suspend-метод
  `transport.send` (`PacketTransport`, `FileTransferTransport.kt:22`):
  «suspend function can only be called from a coroutine or another suspend
  function». Тот же набор файлов из v11.70.26 (тег собрался) этой ошибки
  не даёт - значит ошибка новая, а не унаследованная.
  Лечение: `private suspend fun sendFcap` (единственный вызов - внутри
  suspend `handleOffer`). Правка ушла PR #9; после неё весь набор
  исходников Android (273 файла) компилируется без НИ ОДНОЙ новой
  диагностики относительно v11.70.26 - сравнение нормированных ошибок
  по всем файлам, а не по диффу.
  Правила для следующих сессий:
  1) нельзя ставить тег, ни разу не скомпилировав новый Kotlin-код: PR-гейт
     `ci-core-check` компилирует только Rust, Android не трогает, поэтому
     тег - первое место, где новый Kotlin видит компилятор;
  2) локальный компилятор Kotlin в песочнице (npm + jdk4py) проверяет новый
     Kotlin ДО тега, даже когда Android-сборка недоступна;
  3) сравнивать «новое дерево против последнего собранного тега» по всему
     набору файлов: шумовые диагностики неполной компиляции совпадают, а
     настоящая поломка видна как единственная новая строка.
  Тег v11.70.27 не переставлялся (он указывает на сломанный коммит);
  релиз выпущен следующим номером - v11.70.28.

- **2026-09-18 (раунд 114) — наследование владения в группах и каналах:
  передача прав администратору и захват у удалившегося владельца.**
  Просьба владельца из `AI_HANDOFF.md` пункт 10 (2026-09-11, «эту тему
  потом»): старые аккаунты удаляются, но группа или канал оставались с
  мёртвым владельцем навсегда - ни выйти, ни управлять. Сделано по
  наброску из того же пункта, без правок ядра (только Kotlin):
  - новый пакет `own` в `GroupWire`: `give` (добровольная передача) и
    `take` (наследование); старые сборки молча отбрасывают;
  - чистые правила - `GroupOwnership.kt`: админ забирает владение после
    90 дней молчания владельца, участник (если админов нет) - после 180;
    спор наследников решает меньший nodeId; владельца, которого телефон
    ни разу не видел, молчавшим не считаем - вступивший по ссылке не
    унесёт права живого канала;
  - `GroupRepository`: `transferOwnership`, `claimOwnership`, приём `own`
    с проверками по своей копии состава; принятая заявка передаётся
    дальше один раз; смена владельца - UPDATE строки группы и рассылка
    `roster` с новыми ролями (бывший владелец становится админом со
    всеми правами);
  - UI: во вкладке «Администраторы» кнопка «Передать владение» (у
    владельца, с подтверждением) и карточка «Стать владельцем», когда
    срок прошёл;
  - тесты: `GroupOwnershipTest` (6) и два теста пакета в `GroupWireTest`
    (всего в двух классах 80, прогнаны локальным компилятором Kotlin в
    песочнице - 0 провалов; стаб JUnit на минималках, потому что Maven
    из песочницы недоступен). Полная компиляция дерева против базового
    коммита - без новых диагностика. На телефонах НЕ проверено.
  Релиз не выпускался (нужно разрешение владельца); работа в ветке
  `arena/01a0b524-apumir`.

- **2026-09-18 (раунд 115) — v11.73.0 выпущен для всех (наследование
  владения); грабля: удаление тега переводит релиз в draft.**
  Владелец разрешил релиз («выпускай релиз для всех, сначала зелёный,
  потом слияния»). Порядок и итог:
  - тег `v11.73.0` поставлен на ветку сессии СРАЗУ (без слияния main) —
    так просил владелец: сначала зелёный релиз, потом слияния. Прогон
    35368090384 собрал APK с первого раза (11 м 43 с, success); локальная
    проверка Kotlin накануне (80 тестов, нормированный диф против базы)
    снова спасла от «первого контакта нового кода с компилятором в теге»;
  - промоушен по `promote-release.ps1`: заметки из
    `RELEASE_NOTES_v11.73.0.md`, prerelease=false, перепубликация
    (draft-танец), `/releases/latest` = v11.73.0. apk 39 037 190 Б,
    sha256 `b0d99ab3…89c6`;
  - слияние в main: PR #14, merge ребейзом — GitHub ПЕРЕСОЗДАЛ коммиты
    (main `af3a23b` против ветки `bb1c114`), деревья бит-в-бит равны.
    В прошлый раз (PR #13) SHA сохранялись — на сохранение SHA надеяться
    нельзя, после merge всегда сверять `git rev-parse`;
  - **грабля:** чтобы тег остался достижимым с main (локальные сборки
    берут версию из ближайшего тега), тег переставили: удалили старый,
    поставили на `af3a23b`. GitHub при этом перевёл ОПУБЛИКОВАННЫЙ релиз
    в draft (URL стал `untagged-…`), и `/releases/latest` откатился на
    v11.72.0 — телефоны на несколько минут не видели обновление.
    Вылечено повторной публикацией (`gh release edit --draft=false`).
    Правило на будущее: тег выпущенного релиза НЕ ТРОГАТЬ; если нужно,
    чтобы он был на кончике main, — сначала merge, потом тег на кончик
    main (как делали с v11.72.0), а не наоборот;
  - повторный прогон после перестановки тега (35369513179) корректно
    пропустил сборку: check-job видит существующий релиз. CI-механика
    «пропустить, если релиз есть» работает и на перестановку тега.
  main = `af3a23b` (тег + заметки). На телефонах НЕ проверено.

- **2026-09-19 (раунд 116) — линия v11.74.5…v11.74.9: адрес в конверте,
  свой MQTT-брокер, раздел «Сервер», двухуровневые копии азбуки.**
  Четыре релиза за день, все — по правилу владельца «сначала зелёная
  сборка, потом спросить»:
  - **v11.74.5 «адрес в конверте»:** presence-адрес (ppres) едет ВНУТРИ
    offline-relay сообщения — после смены сети Аня получает ответы
    сразу, без отдельного обмена presence.
  - **v11.74.6 расщепление роя:** диагноз «Аня отправляет — доходит, ей
    отправляют — висит»: прямой клиент шёл на hivemq, WSS-мост — на
    emqx, брокеры не федерируются. Мост и прямой сведены на один
    брокер (hivemq первым), телефон на мосту раз в минуту CONNACK-
    пробой проверяет прямой путь и возвращается, ревизия стабильности
    30→5 мин; карточка «Раздаю обновление» больше не мигает
    (самосев достраивается, не стирается).
  - **v11.74.7 свой MQTT-брокер:** DO MqttBridge = мини-MQTT 3.1.1
    (CONNECT/CONNACK, SUBSCRIBE + retained, PUBLISH fanout без эха,
    PUBACK, LastWill, ping, wildcards, лимиты 400 клиентов/256 КБ),
    101-ответ эхоит subprotocol «mqtt» (требование rumqttc). Ядро: наш
    брокер первый выбор, ConnAck-таймаут 20 с → час на публичных →
    снова наш; заодно висящий старт сессии больше не подвешивает
    движок. Владелец дал CF-токен; первый деплой упал (10079 «Actor
    migration tag precondition failed» — миграция уже применена первым
    деплоем моста), скрипт научен ретраить без миграций.
  - **v11.74.8 копия азбуки на сервере + «Сервер» в настройках:**
    владелец: «резервные копии азбуки адресов это самое главное» и
    «в настройках сделать вкладку "сервер"» (веб-панель не нужна).
    Worker: /addrbook/put|get (полка addrbook|<node_id>, ≤200 КБ).
    Kotlin: AddressBookBackup — AES/GCM ключом из приватного ключа
    узла, автокопия (каждый час при изменении, первая через 3 мин),
    восстановление ДО RustBridge.initialize (ядро читает файл один
    раз), ручные кнопки. Настройки: раздел «Сервер» (замер /health,
    строка роя переехала из «О приложении», копия с диалогом).
    Грабли: (а) decrypt возвращает байты — JSONObject/writeText требуют
    String(UTF-8): первый пробник упал, ошибку вытащили из АННОТАЦИЙ
    check-run'а (логи Actions из песочницы нечитаемы — blob-
    хранилище режет TLS); (б) деплой-скрипт из тега v11.74.8 качал
    worker от v11.74.7 (ссылку не подняли) — «создать копию» отвечало
    «сервер недоступен»; скрипт исправлен: URL=v11.74.8 + жёсткое
    прерывание, если в скачанном файле нет addrbook/put. Владелец
    передеплоил (792 строки, «OK: /addrbook endpoints present»).
  - **v11.74.9 копии азбуки на 5 случайных телефонах роя:** владелец:
    «зачем ограничиваться только на своих… сохранять рандомно, так
    больше шансов; не привязываться к рейтингу». Устойчивая жеребьёвка
    (sha256(мой|кандидат) → топ-5, выпавший заменяется следующим по
    жребию), без рейтинга (решение владельца), конверт тот же
    APUADDBK1 (хранитель байтов не читает), протокол APUBK1
    store/ack/ask/give поверх обычной 1:1 доставки (как APUREAD1,
    routeIncoming до автоконтактов), досылка ≤ раза в 12 ч, у
    хранителя ≤20 владельцев/TTL 60 дней/≤120 КБ, «Восстановить» при
    пустом сервере спрашивает рой (до 40 знакомых, 20 с). Пробник
    зелёный с первого раза (уроки аннотаций пошли впрок).
  - **Проверено владельцем на телефонах:** OPEN mqtt (брокер жив),
    облачная копия создана («прям сразу сохранилось») и восстановлена
    (кнопка «Восстановить» работает), рой-копия «роздана на 4», счётчик
    0-1 — норма при смешанных версиях: подтверждают только хранители на
    v11.74.9. Долги владельца: удалить ОБА CF-токена, обновить все
    телефоны до v11.74.9, снова «Создать копию» (ждём 4-5 из 5).
  Latest = v11.74.9 (d0c4b2f), apk 40 223 910 Б, sha256 348be680…
  36112. Работал в ветке `arena/01a0b575-apumir`; PR #17 оставлен
  открытым (решение владельца).

- **2026-09-19 (раунд 116-бис) — v11.74.10 выпущен: непрочитанные
  комментарии на каждом посте канала.** Просьба владельца: значок не
  только на канале, но и у комментариев каждого поста. Механика уже
  существовала (GroupTopicEntity.unreadCount: инкремент GroupRepository
  на каждом чужом сообщении темы, сброс GroupChatViewModel.markRead при
  чтении; комментарии канала открываются GroupChatScreen по
  topic-маршруту) — добавлены только ChannelPost.unreadComments и
  значок UnreadBadge у кнопки «Комментарии» (тот же золотой стиль,
  кап 99+). Сетевого трафика ноль. Грабля: composable вставлен перед
  шапкой файла (выше импортов) — «imports are only allowed in the
  beginning of file» (check18), перенесён в конец файла (check19
  зелёный). Значок на канале = сумма по постам; новый пост показывает
  «1» (сам пост), пока не откроют комментарии. Latest = v11.74.10
  (e154d85), apk 40 224 422 Б, sha256 25691f9d…06d6. Владелец
  разрешил релиз явно.

- **2026-09-19 (раунд 116-тер) — v11.74.11 выпущен: «Наш сервер» вместо
  технического адреса в настройках.** Владелец: «в настройках в разделе
  сервер нужно убрать конфиденциальную информацию 1985vzhem… назвать
  по-другому». Строка раздела «Сервер» теперь «Наш сервер» (статус и
  время отклика на месте); адрес в коде не менялся (REGISTRY_URL и
  ссылки приглашений прежние — они рабочие, на экран не выводятся).
  Пробник check20 зелёный, владелец разрешил, релиз собрался с первого
  раза. Latest = v11.74.11 (fa3aa04), apk 40 224 422 Б, sha256
  9d945353…847d. ПРАВИЛО: домен владельца в интерфейс не показывать.

- **2026-09-19 (раунд 116-кватро) — v11.74.12 выпущен: тёмная тема в
  Обзоре группы.** Владелец: «в темной теме в группе в настройках в
  обзор названия и описание не видно текста». Диагноз: ApuBubble всегда
  светлый (0xFFF5F7FA) и красит Text через LocalContentColor, но
  OutlinedTextField цвета темы ИГНОРИРУЕТ для LocalContentColor - в
  тёмной теме текст/подпись белели на светлом пузыре. Фикс: полям
  «Название»/«Описание» заданы фиксированные цвета
  (bubbleFieldColors(): текст 0xFF1E2430, подпись 0xFF5A6472, курсор/
  рамка - primary). Поле поиска участников (на обоях) и поля диалогов
  НЕ тронуты - там тема уместна. ПРАВИЛО: любое поле ввода внутри
  светлого пузыря - с фиксированными цветами. Пробник check21 зелёный,
  релиз собрался с первого раза. Latest = v11.74.12 (8e79366), apk
  40 224 422 Б, sha256 40b3823d…cf28.

- **2026-09-20 (раунд 117) — v11.74.13 выпущен: тап по уведомлению ведёт
  в место сообщения.** Владелец: «при нажатии переходило в то место где
  было написано сообщение». Раньше NotificationHelper клал EXTRA_CHAT_ID
  в интент, но НИКТО его не читал — тап просто открывал приложение.
  Теперь: в интент кладутся chatId+topicId (тема группы/пост канала),
  MainActivity.handleNotificationTap читает их в onCreate И onNewIntent
  (extras удаляет — поворот экрана не ведёт повторно) ->
  pendingChatLink -> NavGraph.LaunchedEffect решает по базам через
  ChatLinkEntryPoint (groupDao/chatDao/notificationHelper): группа/канал
  -> GroupChat.createTopicRoute (или весь чат без темы), личный чат ->
  ChatDetail (contactName из ChatEntity); навигация launchSingleTop,
  уведомление снимается. Три пробника на отладку: (1) check22 —
  фантомная «Argument type mismatch: actual String, expected String»
  на ВСЕХ аргументах вызова — оказалось КОНЕВОЙ СИМПТОМ: (2) check23
  «Unresolved reference topicId» — observeAllMessages() возвращает
  DOMAIN-модель Message (domain/model/Message.kt), а не MessageEntity,
  в домене темы не было; (3) check24 зелёный: Message.topicId (default
  null) + toDomain прокидывает. Урок: у ChatRepository методы отдают
  domain-модели; новое поле сущности надо прокидывать в toDomain.
  Latest = v11.74.13 (441fbfc), apk 40 224 422 Б, sha256 44a5e0ba…f052.
  Владелец разрешил релиз. До этого в том же чате: v11.74.12 (тёмная
  тема в Обзоре группы — владелец подтвердил «текст теперь виден»).

- **2026-09-20 (раунд 118) — v11.74.14 выпущен: GIF в темах групп и
  каналов.** Владелец: «чтобы абоненты могли оставлять гифки», потом
  выбрал вариант «добавить ещё каталог GIF». Два слоя:
  (1) АНИМАЦИЯ: coil-gif 2.7.0 + MessengerApplication (ImageLoaderFactory,
  GifDecoder + ImageDecoderDecoder на API 28+); GroupFileCard и
  PhotoViewer крутят GIF через AsyncImage (рамка зума в полном экране по
  bounds-only decode). GroupFileMarker.isGif - MIME или .gif по имени.
  (2) КАТАЛОГ: кнопка «GIF» у скрепки -> GifCatalogDialog (поиск, сетка
  3 колонки, «Ещё»); запросы через НАШ worker /gif/search -> Tenor v2
  (ключ в binding TENOR_KEY; деплой-скрипт читает необязательный
  tenor-key.txt; нет ключа - честная «не настроен»); выбранная гифка
  качается с CDN (BotApi.downloadGif, кап 25 МБ) и едет КАК ФАЙЛ через
  рой (GroupFileSwarm.stageGifBytes). Внешняя точка - только каталог.
  Грабли отладки: (а) Coil 2.7 декодеры через .Factory(), не конструкторы
  (check25); (б) nullability gifBounds в локальной функции (check26);
  (в) ПЕРЕСБОРКА ПЕСОЧНИЦЫ откатила git к 860b47b при живых файлах -
  коммит получился «параллельной историей» (форс-пуш), теги релизов
  v11.74.5-13 НЕ пострадали (сидят на своих коммитах), но ветка теперь
  несёт полный лес, а не цепочку; (г) блок каталога оказался вне функции
  (check28: Unresolved reference viewModel) + съеденная скобка
  MessageBubble - перенесён после showGifCatalog, скобка возвращена
  (check29 зелёный). Latest = v11.74.14 (2190da1), apk 40 257 190 Б,
  sha256 9b7062ab…3ca0. Владелец подтвердил выпуск. КАТАЛОГ ЗАРАБОТАЕТ
  после того, как владелец положит tenor-key.txt и повторит деплой.

- **2026-09-21 (раунд 118-бис) — воркер: каталог GIF умеет Giphy.**
  Google Cloud Console у владельца стабильно не отдаёт страницу Library
  (телефон + ПК, Retry не помогает) - Tenor-ключ недоступен. Обходная
  ссылка flows/enableapi дана, но параллельно воркер научен Giphy:
  GIPHY_KEY имеет приоритет, TENOR_KEY остался запасным; формат
...
- **2026-09-21 (раунд 119) — v11.74.16: GIF-кнопка в личке +
  потеря сообщений после обновления.** Владелец: (а) GIF-кнопка и
  скрепка наложились в личке; (б) «отправить сообщение - получатель
  обновил приложение - не может получить». Разбор (б) нашёл ДВЕ дычи,
  обе в ядре: (1) EventBus (events.rs, cap 1000) при переполнении
  выкидывал САМОЕ СТАРОЕ событие; после обновления Kotlin начинает
  drain с опозданием, ядро льёт retained-presence/gossip/куски -
  MessageReceived выбрасывался; (2) MESH relay: durable tombstone
  (apu_relay.sqlite) писался в момент UI-доставки; если процесс умирал
  (обновление!) до сохранения в Room, повторная доставка давилась
  tombstone-ом + receipt уходил - отправитель видел «доставлено»,
  сообщение пропадало навсегда. Фиксы: eviction_rank (0=сообщения/ACK
  никогда, 1=куски/статусы, 2=погода сети; иначе мягкий рост) в
  events.rs + tombstone больше НЕ блокирует UI-доставку в core.rs
  (дубли фильтрует Room messageExists + повторный ACK). +4 теста
  (flood-переживание). Урок: cargo в песочнице нет (rustup/apt не
  ставятся, TLS-резка) - проверка только CI; check33 зелёный сразу.
  v11.74.16 = fe724d8, APK 40 271 046 Б, sha256 2566e60a…0b0.

- **2026-09-22 (раунд 120) — v11.74.17: четыре полевых замечания по
  гифкам.** Владелец после испытаний .16: (1) реакцию на гифке никто
  не может поставить; (2) у отправителя нет анимации; (3) «Приложение
  не отвечает» (ANR) при отправке; (4) повтор той же гифки гоняет весь
  объём заново. Причины/фиксы: (1) FileTransferBubble в личке имел
  только меню файла (и лишь у принятых) - добавлен onLongPress ->
  showCopyDialog («Поставить реакцию») + ReactionRow под пузырём;
  группы были в порядке с .15. (2) превью исходящей картинки писалось
  .jpg (PhotoPreview) - для image/gif пишем копию гифки как
  <transferId>.gif; previewFileFor читает .gif первым (старые - .jpg).
  (3) pumpOutgoing звали из ViewModel на Main: внутри блокирующие
  QUIC-отправки -> ANR; теперь withContext(IO) внутри самого насоса +
  запись tmp-гифки в IO. (4) у получателя: если OFFER несёт файл, что
  уже принят (COMPLETE, тот же sha256+отправитель+чат, срок жив) -
  completeFromLocalCopy копирует проверенный plaintext в новый
  transferId, сразу COMPLETE + полный ACK (DAO
  getCompletedIncomingSameFile); отправитель останавливается на первом
  ACK, куски до ACK игнорируются штатно (ingestChunkCiphertext умеет
  COMPLETE). Ключевой факт: messageId файла у получателя =
  chatPlaceholderMessageId(transferId), поэтому дедуп возможен только
  новым transferId + локальной копией, а не переиспользованием строки.
  Песочница ПЕРЕСОБРАЛАСЬ между ходами: HEAD уехал на 860b47b, всё
...
- **2026-09-22 (раунд 122) — v11.74.19: гифка из роя у ТРЁХ хранителей.**
  Вопрос владельца: «система работает в личке/группах/каналах/комментариях?
  котик на 10 телефонах - прислать быстро из 10 мест?» Ответы: да, везде,
  где кнопка GIF (комментарии/каналы = тот же GroupChatScreen). Мультизакачка:
  в группах K2 уже качает полосами с нескольких сидов; в личке было
  1-хранитель -> сделал requestGif: want ТРОИМ лучшим (PeerRatingStore:
  fresh lastSeen/sightings), тротлимб 2 мин/sha (wantSentAt), статус
  «Попросил у X (и ещё 2)» ("" = уже просили). Приёмник: DAO
  getActiveIncomingSameFile (INCOMING, не COMPLETE/FAILED/CANCELLED, живой
  срок) + отказ CANCEL-ом дублю в полёте (updated за 10 мин), строго внутри
...
- **2026-09-22 (раунд 125) - v11.74.22: азбука без лимитов копий +
  слияние присланных в ОДНУ.** Владелец: «копий сколько угодно где; сам
  телефон держит одну - прислали от 10 участников, он в свою добавил
  тех, кого нет». (1) AddressBookSwarmBackup: MAX_OWNERS=20 и
  OWNER_TTL_MS=60д УДАЛЕНЫ (storeBlob не вытесняет; у владельца файл
  один - свежая копия; HOLDER_COUNT=5 остался - это ширина раздачи,
  решение владельца «хотя бы 5»). (2) Слияние: askAndRestore ->
  askAndRestoreAll (ConcurrentLinkedQueue, валидные конверты до 4
  версий/20с), AddressBookBackup.mergeIntoLocalBook: добавляет ТОЛЬКО
  id, которых нет локально; restoreNowForce больше НЕ замещает файл
  (было - замещал), askSwarm сливает все версии. Сообщения: «добавлено
...
- **2026-09-22 (раунд 127) - v11.74.24: ЕДИНЫЙ каталог гифок + синяя
  переливающаяся точка «уже в нашей сети».** Владелец: «каталог единый
  что внешний что внутренней сети; выбираешь любую - приложение само
  определяет есть в сети или нет; есть - синяя переливающаяся точка
  в правом верхнем углу; ниже пометь что означает». GifCatalogDialog
  переписан: вкладок НЕТ, одна сетка = мои (бейдж «мой» + превью) ->
  сеть (бейдж «у N», БЕЗ дублей по giphyId если она видна во внешних/
  моих ячейках) -> внешние. Сопоставление внешней гифки с сетью ПО
  giphyId (он расходится в have-пакетах APUGIF1 и хранится в
  GifLibEntry). Точка: NetDot = rememberInfiniteTransition, пульс alpha
  0.35..1 + перелив цвета 2F80ED..8FC1FF, 650ms Reverse, белая кайма.
  Клик по внешней с точкой: myEntry -> onAttachLocal, peer ->
  onRequestSwarm (наружно НЕ качается). Легенда под сеткой. «+ Своя»
  в строке поиска. ГРАБЛИ: InfiniteTransition.animateColor -
...
- **2026-09-23 (раунд 131) - v11.74.28: доводка гифок-ссылок по
  скриншотам.** Владелец (оба телефона .27): «приходит только ссылка
  в уведомление; в группе получить никто не может». Разбор: карточка
  ВСТАВЛЯЕТСЯ (insertReceivedGifRefMessage), но (а) НАБЛЮДАТЕЛЬ
  УВЕДОМЛЕНИЙ (CoreServerService.startEventPolling, observeAllMessages,
  «свежие входящие за 2 сек») показывает СЫРОЙ контент - это и было
  «непонятная ссылка в уведомлении»; (б) байты тянулись только из UI
  (ensureGifRef при открытии чата); (в) НОВАЯ гифка (attachGif скачал
  из внешнего каталога) НЕ объявлялась: syncWithSwarm зовётся только
  при открытии окна гифок/старте - участники не знали хранителя
  (swarmCatalog пуст по этому sha -> holders.isEmpty() -> ничего).
  Фиксы: (а) наблюдатель: isGifRef -> «🖼 Гифка»; (б) сервис в
  ref-перехвате после вставки карточки зовёт ensureGifBytesForRef
  (gifFile? -> swarmCatalog holders -> rememberWant + requestGif);
  (в) sendGifRefInternal/sendGifRefToGroup после отправки зовут
  syncWithSwarm (fp изменился - объявит всем). ГРУППЫ: карточка .27
  рисуется (MessageBubble isGifRef), уведомление тоже починено (тот же
  наблюдатель); на <=.26 в группах сырой текст - только обновление.
  Первый скрин владельца (20:58, сырой APUGIFREF1 в уведомлении) -
  смешанные версии. check53 зелёный. v11.74.28 = 48f5ffe, APK
  40 369 350 Б, sha256 22becafe…4e11.

- **2026-09-23 (раунд 132) - v11.74.29: компактные дифф-обновления
  (APUBSP1).** Владелец: «при скачивании обновление должно качаться
  минимального размера». Сделано: формат APUBSP1 (магия, blockSizeBits
  12=4096, u64 размер нового, u32 счётчики, усечённые sha256 блоков
  старого, дескрипторы 5 Б: 1=REF индекс, 2=RAW + длина deflate,
  затем конкатенация raw-deflate); генератор-эталон
  tools/ci/make_apk_patch.py (stdlib), применитель на телефоне
  ApkDiffPatch.kt (RandomAccessFile по sourceDir, Inflater(true));
  UpdateChecker: из digest ассета берёт apkSha256 («sha256:hex»), ищет
  ассет patch-*-to-*.bspatch с to == normalize(latest); если
  patchFrom == версия телефона - качает ТОЛЬКО патч, собирает APK,
  сверяет sha256, открывает установщик (тост «скачано компактно»,
  возврат -1L); иначе - полный APK как раньше (фолбэк), рой-раздача
  не тронута. ВАЖНО: шаг в build-release.yml добавить НЕЛЬЗЯ (GitHub
  App без права workflows), а из песочницы релизные APK и workers.dev
  НЕДОСТУПНЫ - патч собирает ВЛАДЕЛЕЦ одной командой на своём ПК:
  powershell -File tools\ci\make_update_patch.ps1 -FromTag <старый> -ToTag <новый> -Upload
  (PS-скрипт ASCII-only, PS 5.1: gh есть, EAP вокруг native-вызовов
  мягче; генерирует, ОБЯЗАТЕЛЬНО самопроверяет применением+sha256 и
  грузит). Кросс-проверка формата - в ci-core-check.sh (блокирующая,
  python <-> pwsh на синтетике: дубликат блока, изменённые/нетронутые
  блоки, частичный хвост; эталон применителя apply_apk_patch.py).
  check54 красный (Long/Int в ApkDiffPatch.kt:96) -> fix ->
  check55 зелёный. v11.74.29 = e1e9cec, run 35822801542 SUCCESS,
  latest, APK 40 369 350 Б, sha256 4acf7812…8c0a. ПАТЧ-АССЕТ к .29
  ещё НЕ приложен (владелец должен прогнать скрипт:
  -FromTag v11.74.28 -ToTag v11.74.29 -Upload); без патча телефоны
  качают полный APK (фолбэк), это не поломка.

- **2026-09-23 (раунд 133) - v11.74.30: дифф-патч ходит и по рою.**
  Владелец (вечером обновит патч-ассеты): «по рою тоже отправляй только
  маленькую нужную часть». Сделано: uppk (объявление патча:
  от/до версии, sha патча + итогового APK, размер) и uppwant (просьба,
  привязка как у upwant); отказ - прежний upnone (версия «до», sha
  патча). Патч - просто маленький файл в apkseed: та же файловая машина
  (манифест, куски, полосы). Источники патча: (а) компактная загрузка
  с сайта - UpdateChecker БОЛЬШЕ НЕ УДАЛЯЕТ патч, хук onPatchKept ->
  ApkSeeder.adoptPatchFromSite (файл в seedRoot/$sha.bspatch +
  patchseed.v1; копию/объявление достраивает помпа); (б) рой: строка
  INCOMING/COMPLETE патча - источник без перешифровки. Приём:
  onPatchFileComplete - ApkDiffPatch.apply(установленный, патч),
  сверка sha256, карточка «Установить», ПАТЧ остаётся раздачей для
  «от»-телефонов, собранный APK - раздачей для остальных (как
  скачанный с сайта). Настройки: «Скачать компактно v<до> (патч, N)» +
  ход приёма + останов. Чужим «от»-версиям uppk невидим (isSame from);
  не сошлось - фолбэк на целый APK. sweepPatchSeed чистит патчи старее
  моей версии. Апдейт-лог: check56 красный (съеденная сигнатура encode
  в ApkUpdateStore), check57 красный (задвоенные AUTOSEED_* той же
  вставкой - урок: якорь+add повторяет якорь), check58 зелёный.
  v11.74.30 = 8477ca8, run 35829918510 SUCCESS, latest, APK
  40 385 734 Б, sha256 d4969f59…18bd. Патч-ассет к .30 прикладывает
  владелец: make_update_patch.ps1 -FromTag v11.74.29 -ToTag v11.74.30
  -Upload (можно и -FromTag v11.74.28 -ToTag v11.74.30 для прыжка
  через версию).

- **2026-09-23 (раунд 134) - v11.74.31: гифка не задваивается.**
  Владелец (скрин, чат с server7): «Отправил гифку. Но она задвоилась» -
  карточка-ссылка ПЛЮС пузырь «Сохранено ✓» с той же гифкой. Разбор:
  выбрал гифку из каталога СЕТИ (requestSwarmGif) -> sendGifRefInternal
  вставил карточку; своей гифки у телефона не было -> ensureGifRefInternal
  -> rememberWant + requestGif у хранителя; хранитель отдал байты тихо
  (serveGifFromLibrary, mediaType image/gif, у себя строки нет), но
  НОТИФИКАТОР FileTransferRouter на COMPLETE входящей передачи вставлял
  saveIncomingMessage-плейсхолдер - «вторую карточку». Фикс: плейсхолдер
  НЕ вставляется, если mediaType image/gif И GifLibrary.isWanted(sha)
  (этот телефон сам просил байты; скрепочный .gif чужой - пузырь
  остаётся). Хук «байты -> библиотека» (addFromFile) не тронут - от него
  оживает карточка-ссылка. ГРАБЛЯ САНДБОКСЫ: пересборка откатила HEAD
  на 860b47b (v11.73.0), фикс-коммит лёг на чужой базу - спасено
  копией файла в /tmp, fetch + reset --hard на d1ef589, фикс
  перекоммичен начисто (0811d83). check59 зелёный. v11.74.31 =
  0811d83, run 35839581858 SUCCESS, latest, APK 40 385 734 Б,
  sha256 1d5b3452…8c22. Старые задвоенные карточки в истории чатов
  остаются (чистки БД нет).

- **2026-09-23 (раунд 135) - v11.74.32: удаление своих сообщений.**
  Владелец: «в личных сообщениях и в группах и каналах в комментариях
  чтобы можно было удалить свое сообщение у себя и выбор - удалить у
  всех». Сделано: долгое нажатие на СВОЁМ сообщении -> «Удалить у себя»
  (локально) и «Удалить у всех» (с подтверждением). Личка: конверт
  APUDEL1|chatId|messageId|atMs (MessageDeletionRepository, маршрут в
  CoreServerService ПОСЛЕ реакций, до постов) - получатель стирает
  через MessageDao.deleteByIdChatAndSender (id+chatId+senderId -
  подделка чужого удаления невозможна; свой chatId ищем по отправителю,
  как у реакций). Группы/комментарии: пакет msdel (GroupWire
  buildMessageDelete/Packet.MessageDelete), приём ВНУТРИ GroupRepository
  по шаблону Edit (права: автор ИЛИ владелец), отправка через broadcast.
  Куски InlineImage-хвостов и реакции стираются с головой; превью
  списка чатов пересчитывается (MessageDao.getLatest; gif -> «🖼 Гифка»,
  фото -> «Фото», как GroupRepository.preview). Собеседник недоступен -
  честный отказ без половинного удаления. Оба экрана: пункты красным
  (colorScheme.error), GroupChatScreen - dropdown в MessageBubble +
  confirm-диалог на экране, ChatDetailScreen - в actions-диалоге +
  confirm. check60 зелёный С ПЕРВОГО раза. v11.74.32 = ca1b671, run
  35843939903 SUCCESS, latest, APK 40 402 118 Б, sha256 970bdf3e…8a68.
  Ограничения: у телефонов старых версий сообщение останется (msdel/
  APUDEL1 молча отбрасываются); превью групп/тем после удаления не
  пересчитывается (как у Edit); переданные файлы не чистятся (только
  строка сообщения).

- **2026-09-23 (раунд 136) - v11.74.33: окно обновления без «кода»**.
  Владелец (скрин): «В объявлении на обновление написан какой то код» -
  UpdateDialog показывал ТЕЛО релиза как есть (markdown: ##, **, блок
  powershell). Фикс: UpdateChecker.humanNotes() - заголовок + ПЕРВАЯ
  секция заметок (до второго «\n## »), снять разметку (#, **, `),
  обрезка по границе слова на 420 символах + «…»; пусто - блок «Что
  нового» скрыт. ОБА пути (GitHub body и relay notes) через одну
  очистку. СОГЛАШЕНИЕ О ЗАМЕТКАХ: первая секция - только человеческий
  текст; команды/патчи - после заголовков «## » (они в окно не едут).
  check61 зелёный. v11.74.33 = 4171648, run 35850079755 SUCCESS,
  latest, APK 40 402 118 Б, sha256 c5add7f3…9d26.

- **2026-09-26 (раунд 180) - v11.74.77: аудит-2.**
  Оба кандидата из AI_HANDOFF сделаны: markOnline файлов только на
  тяжёлом пульсе (!lightTouch, раз в 30 с); кэш сжатого аватара
  (GroupsModule + MyAvatarMemo, сжатие только при смене uri).
  Поведение/E2E не тронуты. Грабли: объект-кэш вставлен до импортов -
  check133 fail, перенос в конец файла, check134 success. v11.74.77 =
  4ca51eb, run 36267512069, APK 40 582 294, digest 428345ed…e92,
  latest.

- **2026-09-26 (раунд 179) - v11.74.76: авто-слив очереди.**
  Владелец: гифка у отправителя есть, у получателя НЕ появляется.
  Причина: досыл только в peer_discovered после lightTouch - при живой
  связи пульсы чаще 30 с всегда лёгкие, retryPending не звался вообще.
  Фикс: MessageDao.getQueuedOfflineMessages + ChatRepository
  .pumpQueuedOffline(20) + шаг в filePumpJob (20 с цикл, слив раз в
  минуту). Дубли сняты дедупликацией; пустая очередь = один SELECT.
  v11.74.76 = 21fb669, run 36264170367, APK 40 582 294, digest
  b40d0f42…55b, latest.

- **2026-09-26 (раунд 178) - v11.74.75: гифка не ждёт + аудит-1.**
  Владелец: гифка отправляется не дожидаясь собеседника; доставка сама
  при появлении; запас - рой. sendGifRefInternal: без check(sent);
  карточка всегда в чате, при неудаче статус QUEUED_OFFLINE; досыл -
  существующий конвейер retryPending/FULL SYNC по presence; байты - из
  роя (ensureGifRef). Аудит-1 (только безопасное): публичный IP по
  кэшу 10 мин. Грабли: ЧЕТВЁРТЫЙ захват чужого WIP (база 860b47b,
  коммит поймал 960 вставок, push отлонён) - восстановлен reset на
  origin tip + поимённый коммит; правило: сверять HEAD перед КАЖДЫМ
  коммитом, не только перед тегом. v11.74.75 = 3a52bb4, run
  36260701348, APK 40 565 910, digest fa09f8f8…c03, latest.

- **2026-09-26 (раунды 176-177) - v11.74.73/74: карточка контакта +
  стабильное имя профиля.** р176 (v11.74.73 = 6c78146, run 36255915348):
  приглашение контакта рисуется карточкой (ContactCardSender.parseCard +
  MessageBubble.ContactInviteCardView): имя, @ник, золотая кнопка
  «Добавить контакт» -> Screen.AddContact с Uri.encode(link). Причина:
  URL_REGEX ловил только http(s) - apu:// был некликабелен. В контактах
  убрана дублирующая иконка «поделиться» (onShareClick). Грабли: апдейт
  снова лёг на 860b47b (чужой сброс, патч восстановлен до коммита);
  неверный import ApuLink (util, не data.group) - пойман до пробника.
  р177 (v11.74.74 = bae557e, run 36258043088): «анонимус» в профиле -
  SettingsUiState.displayName по умолчанию Anonymous; фикс: init читает
  p2p_prefs/display_name синхронно до первого кадра (одно чтение кэша,
  нагрузка ~0). Владелец просил снижать нагрузку для слабых телефонов -
  кандидат в следующий раунд: loadSettings дёргает https api.ipify.org
  при каждом открытии вкладки.

- **2026-09-26 (раунд 175) - v11.74.72: поделиться контактом в APU +
  имя без цифр.** Обе задачи владельца: «Поделиться контактом» →
  «Кому отправить» из «Контактов» И из списка чатов. ContactCardSender
  шлёт apu://-ссылку контакта (ContactShareLink) сообщением; получатель
  тапает - «Добавить контакт». Fix «имя с цифрами»: whois-ответ видимым
  именем при пустом @нике (registeredAtMs=0, настоящие ники не бьёт) +
  whois-запрос при входящем от безымянного (main и CF пути). Грабли:
  параллельная сессия дважды сносила worktree (восстановление
  checkout bd0bd39 -- + python-патч; edit_file-fuzzy вставил мусор в
  ChatListScreen - struct_check поймал); check127 Unresolved
  shareCardFor -> onShareCard-колбэк через SectionPage; check128
  success. v11.74.72 = a6c8d37, run 36250429013, APK 40 565 910,
  digest 9a2c1114…bdc, latest.

- **2026-09-26 (раунд 174) - v11.74.71: удаление своего из роя.**
  Владелец: «удалить стикер или гифку из роя, который случайно
  закинул, хотя бы пока никто не скачал». StickerLibrary.deleteBySha +
  GifLibrary.deleteOwn (индекс + файлы); долгое нажатие в панели на
  «Мои» стикеры/гифки → подтверждение с честным текстом; после
  удаления force-sync - каталог переобъявляется, у абонентов
  исчезает из «Из сети»; скачанное не отзывается (E2E). Грабли:
  combinedClickable = experimental (нужен @OptIn ExperimentalFoundationApi)
  - check125 fail → check126 success. ИНЦИДЕНТ: worktree переклонирован
  параллельной сессией, первый тег встал на 860b47b - релиз/тег снесены,
  перед тегом теперь ЖЁСТКОЕ правило: сверять HEAD sha с ожидаемым
  перед git tag. v11.74.71 = c83d119, run 36241664307, APK 40 565 910,
  digest 6ba84885…5296, latest.

- **2026-09-26 (раунды 172-173) - v11.74.69/70: стикеры+закрепы.**
  р172 (v11.74.69 = df4a678, run 36230742116, APK 40 549 526): стикер
  в личке рисуется всегда (чёрный квадрат = jpg-снимок webm; порядок:
  библиотечный файл -> роутер), меню/удаление у стикеров
  (canActOnImage без previewBitmap), StickerViewer - увеличение с
  непрерывной анимацией (личка/группы/Избранное), пикер стикеров
  «*/*» (видит webm), «Из сети» анимирует локально доступные
  (libraryFile companion), закреп группы -> переход. р173 (v11.74.70 =
  62b84d2, run 36234387005, APK 40 549 522): закрепы лички и каналов:
  dao observePinnedChatMessages/observePinnedChannelPosts,
  Message.isPinned в домене (грабли check123: не было в домене,
  иконки без импортов, viewModel вне PostCard - колбэк), бары с
  переходом, булавка поста канала. Избранное - отложено (миграция БД,
  fallbackToDestructive - осторожно). check124 success; оба latest.

- **2026-09-26 (раунд 171) - v11.74.68: фикс ANR, оптимизация.**
  Владелец: 11.74.66 зависает («Приложение не отвечает», скрин) и не
  даёт обновиться; «проверь все процессы и сделай оптимизацию».
  ПРИЧИНА: NetworkChangeReceiver.onReceive (main!) при каждой смене
  сети stop+start сервиса; ночью сеть прыгала - шторм пересозданий +
  офлайн QUIC-таймауты 5-10 с/send. Фиксы: дебаунс 10 с + goAsync-
  перезапуск в фоне; RustBridge.isNetworkUp + дедлайн 8 с в
  StickerLibrary/GifLibrary.syncWithSwarm (офлайн не стартуем);
  readBytes/touch стикеров - в IO. Аудит чист: текст/файлы уже IO
  (р120), pingHealth IO+таймауты, AvatarBitmaps IO/Default. ГРАБЛИ:
  у ресивера был СВОЙ companion (check117); локальный репозиторий
  опять пересобран на базу - кончик origin восстанавливать ПЕРЕД
  работой, патчи повторно по-месту. check118 success. v11.74.68 =
  b9fbbcc, run 36218334271 SUCCESS, latest, APK 40 533 142 Б.

- **2026-09-25 (раунд 170) - v11.74.67: webm-стикеры, анимация везде.**
  Владелец: анимации не работают нигде; панель не закрывается после
  отправки; кнопки добавить - наверх. ПРИЧИНА: vibesticker WEBP -
  статичный формат (анимация у них только WEBM). Решение: zip-импорт
  принимает webm; НОВЫЙ StickerAnimated.kt - webm покадрово
  (MediaMetadataRetriever, ≤12 кадров 256px, LRU 6, 90 мс, хромакей
  порог 38), остальное - Coil; ячейки панели/GroupFileCard/
  FileTransferBubble/SavedScreen переведены на него; гейты превью
  (FileCardState, Router.previewFileFor + библиотечный fallback
  исходящих, тихий фильтр лички, парящий пузырь) расширены на
  «Стикер…»; stageStickerBytes/личный sendSticker шлют webm с честным
  типом; панель закрывается после отправки; кнопки «+» наверх сетки,
  индексы секций точные (recents=2, my=3+max(recents,1),
  swarm=my+1+stickers). ГРАБЛИ: самописный LZW GIF-энкодер НЕ
  использовать - правила роста кода не выверить без устройства
  (python-прототип расходился с PIL); кадры webm от retriever - без
  альфы, прозрачность только хромакеем. check115 success сразу.
  v11.74.67 = 2e61de9, run 36187737278 SUCCESS, latest, APK
  40 533 142 Б.

- **2026-09-25 (раунд 169) - v11.74.66: стикеры без фона, парят.**
  Владелец: убрать квадратный фон, стикер «как будто в чате», анимация
  во всех чатах. GroupFileMarker.isSticker; GroupFileCard - стикер без
  подложки/скруглений; MessageBubble - Card Transparent+elevation 0;
  FileTransferBubble - без подложки, «Получено ✓» скрыта; ApuBubble -
  параметр transparent, SavedScreen - стикеры без пузыря и анимацией
  (isLiveItem: gif|webp). Декодер webp уже был (ImageDecoderDecoder).
  ИНЦИДЕНТ: локальный репозиторий опять пересобран на базе 860b47b -
  спасать правки в /tmp ДО reset, сверять по-строчно с origin-кончиком,
  коммитить только свои пути. check114 success. v11.74.66 = 57e0619,
  run 36180678918 SUCCESS, latest, APK 40 533 142 Б.

- **2026-09-25 (раунд 168) - v11.74.65: «три точки» на карточке.**
  Владелец (скрин): кнопки «Сохранить в папку/Поделиться/В избранное»
  не помещаются под стикером - сделать три точки в правом верхнем углу
  и три горизонтальных пузыря в гамме APU. GroupFileCard: Column
  обёрнут в Box; круглая кнопка MoreVert (surface 0.88, primary tint,
  32dp) в TopEnd; DropdownMenu с CardActionBubble (primary/onPrimary,
  clip 18, иконка+подпись) x3 - только доступные действия; прежний ряд
  TextButton убран. Личка (раунд 44) и документы без превью - как
  были. ИНЦИДЕНТ: локальный HEAD сброшен параллельной сессией на базу
  860b47b - cp файла в /tmp, reset --hard origin/ветка, возврат,
  коммит только своего пути. check113 success сразу. v11.74.65 =
  4991574, run 36173696548 SUCCESS, latest, APK 40 533 142 Б.

- **2026-09-25 (раунд 167) - v11.74.64: стикеры в рой сразу.**
  Владелец: «стикеры как и гифки - в рое; закачал .zip или по штучно -
  любой абонент просматривает общий каталог со всех устройств роя».
  Роевой каталог был с р139, но SavedViewModel не объявлял каталог
  вовсе, остальные пути ждали 10 мин. StickerLibrary.syncWithSwarm:
  force=true минует троттлинг; все 6 путей добавления (Избранное ×2,
  личка ×2, группы/каналы ×2) зовут announceAdded() сразу после
  записи. ИНЦИДЕНТ: первый коммит раунда смёл незакоммиченный WIP
  параллельной сессии (69 файлов, check110 failure) - лечится
  `git reset --hard <кончик>` + возврат только своих файлов, коммитить
  ТОЛЬКО конкретные пути (никогда `git add <каталог>` целиком);
  check111: announceAdded обязан быть suspend. check112 success.
...
- **2026-09-25 (раунд 161) - v11.74.58: золото повсюду в приглашении.**
  ...
  x3; строка адресата -> пузырь (выбран - primary+White Bold), круглый
  чек 24dp CircleShape (рамка->заливка+Check, inGroup серый disabled),
  spacedBy(6.dp). check98 success С ПЕРВОГО прогона (якорь - сам блок,
  импорты wildcard). v11.74.58 = 1b65f2b, run 36107734721 SUCCESS,
  latest, APK 40 517 078 Б, sha256 ff211512…003.

- **2026-09-25 (раунд 160) - v11.74.57: пузыри приглашения.** Владелец:
  кнопки диалога «Пригласить» - красивые, три горизонтальных пузыря
  друг под другом в нашей гамме. InviteActionBubble(filled): золотая
  заливка/светлый + рамка; диалог: кнопки из confirm/dismiss слотов
  убраны ({}), пузыри в text-Column (QR filled, ссылка, в APU).
  ГРАБЛИ: check95 - text задвоен (старый блок описания не вычищен);
  check96 - функция вставлена в doc-блок SectionPage (двойной
  @Composable, SectionPage потерял аннотацию). УРОК: при замене слотов
  вычищать старые параметры; функции вставлять вне чужих doc (якорь -
  сама fun, не док перед ней). check97 success. v11.74.57 = 3aef539,
  run 36103389197 SUCCESS, latest, APK 40 517 078 Б,
  sha256 890627d2…6cc.

- **2026-09-25 (раунд 159) - v11.74.56: «уже в группе» в выборе.**
  Владелец (скрин диалога р158): участники группы - серым, отмечены,
  «уже в группе», не выбираются. Реализация: ChatListViewModel
  .groupMemberIdsOnce (getMembers -> Set<nodeId>); строка диалога:
  inGroup = contactId in memberIds -> Checkbox(checked=inGroup||выбран,
  enabled=!inGroup), имя серым, суффикс «уже в группе», Row
  clickable(enabled=!inGroup); счётчик и рассылка - только новые.
  13-й откат посреди раунда (правка VM потеряна; р158 в git цел) -
  fetch+reset, повтор патчей. v11.74.56 = 5b81df4, run 36097356780
  SUCCESS, latest, APK 40 517 078 Б, sha256 1d60c41b…4bd.
...
- **2026-09-24 (раунд 157) - v11.74.54: «Поделиться» без GitHub.**
  ...
  (OFFICIAL_HOST в коде, App Links в манифесте) - владелец: «У МЕНЯ
  НЕТ РЕСУРСА apumir.app» -> полный откат на WEB_HOST; installLink =
  <воркер>/update/apk (воркер и так отдавал APK-поток). УРОКИ:
  (1) домен в коде НЕ доказательство владения - спрашивать владельца
  ДО переключения внешних ссылок; (2) OFFICIAL_HOST/ReferralInviteLink
  - под вопросом, владелец решил пока не разбирать (флаг в коде
  оставлен). «1985vzhem» прячется только нейтральным доменом владельца
  (переименование workers.dev subdomain сломает старые телефоны -
  НЕ делать). 11-й откат (281ca9b запушен, правки отката повторно).
  check90/91 success. v11.74.54 = 50a0960, run 36054425368 SUCCESS,
  latest, APK 40 500 694 Б, sha256 dabe9e81…815.

- **2026-09-24 (раунд 156) - v11.74.53: дочистка + тихие конверты.**
  Владелец (скрин «Сообществ»): «в поиске сообщества тоже системные
  ссылки; проверь ещё места». Дочищено: GroupsScreen «Мои группы»
  (append lastMessagePreview), закреп темы (гифка-ссылка сырой),
  текст уведомлений (стикер-конверты). Владелец выбрал «спрячь
  стикер-уведомления»: ChatPreviews.isServiceEnvelope (APUSTK1| +
  APUGIF1|) - фильтр recentIncoming (не звонят) + guard на
  incrementUnread (не считают). APUGIFREF1 - настоящие карточки,
  не тронуты. 10-Й ОТКАТ посреди раунда (HEAD -> 860b47b, дерево
  новое, якорь step-2 съел human() из р155): recovery fetch+reset
  на 67f1650, повтор патча ПОШАГОВО. УРОК: heredoc-питон и следующие
  шаги - ТОЛЬКО через &&; после отката якоря перепроверять по
  фактическому файлу. check88+check89 success. v11.74.53 = 07e6e81,
  run 36044902008 SUCCESS, latest, APK 40 500 694 Б,
  sha256 327bc64f…b73.
...
- **2026-09-24 (раунд 154) - v11.74.51: закрытые группы вне поиска.**
  ...
  появилась у других в поиске». Причины: getOwnPublishable без фильтра
  isPublic (каталог рассылал ВСЕ свои группы со ссылкой);
  matchDirectory не фильтровал needsApproval. Фиксы: DAO + isPublic = 1;
  matchDirectory + !needsApproval (прячет и старые записи, уже
  разошедшиеся по сети). УРОК: «нужен фильтр на обеих сторонах» -
  публикация (источник) и показ (приёмник); проводной формат не меняли
  (isPublic в пакете Directory и не ездил). check85 зелёный.
  v11.74.51 = a1408f7, run 35999729746 SUCCESS, latest, APK
  40 500 694 Б, sha256 de95db2c…c72.

- **2026-09-24 (раунд 153) - v11.74.50: группы без тем + конвертация.**
  Владелец (скрин «Выберите тему» при пустом списке): «в группе без тем
  не пишутся сообщения; нужно в настройках переводить в группу с темами».
  Причины (3 слоя): createGroup(topicsEnabled=false) НЕ создаёт строк тем;
  VM при пустом списке не выбирает тему (лента не стартует, send()
  молча return); resolveTopic -> null -> «Тема не найдена». Фикс:
  ensureFlatTopic - General с ДЕТЕРМИНИРОВАННЫМ id <groupId>:general
  (каждый телефон сам создаёт одинаковую строку - без рассылки!),
  VM.init ждёт group и делает selectTopic(flatId). Конвертация:
  enableTopics (canChangeInfo; DAO setTopicsEnabled; General + broadcast
  buildTopicCreated если тем нет; плоский General остаётся), UI-пузырь
  в OverviewTab. Нюанс: метаданные (topicsEnabled) участникам летят
  только с полным синком - их UI может остаться плоским, но писать
  они могут сразу (TopicCreated -> авто-выбор). check84 зелёный.
  v11.74.50 = 8cddbba, run 35994856497 SUCCESS, latest, APK
  40 500 690 Б, sha256 9c721d11…e7a.

- **2026-09-24 (раунд 152) - v11.74.49: «Отправить» загорается сразу.**
  Владелец (скрин: «р» в поле, кнопка серая): «кнопка отправить долго
  не появляется; нужен выразительный текст И другой фон, когда можно
  отправить». Причины: визуально менялся ТОЛЬКО оттенок текста
  (primary<->серый на полупрозрачном пузыре) - незаметно; в ЛИЧКЕ
  enabled шёл через родителя (snapshotFlow -> onTextChange ->
  recomposition) - реальная задержка. Фиксы: val canSend; активная
  кнопка = primary fill + White Bold, неактивная = surface 0.55 +
  серый; GroupChatScreen (темы) canSend из draft/stagedFile, личка -
  из inputState.text напрямую. УРОК: состояния действий должны читаться
  из ближайшего источника (поле), а не через round-trip; смена
  доступности должна быть КОНТРАСТНОЙ. check83 зелёный.
  v11.74.49 = 4d129f8, run 35990034737 SUCCESS, latest, APK
  40 500 694 Б, sha256 69d5e96a…b69.

- **2026-09-24 (раунд 151) - v11.74.48: лента над клавиатурой.**
  Владелец (скрин: входящие «эээ» просвечивают сквозь пузыри поля,
  клавиатура открыта): «переписка так и прячется за пузырём ввода
  и пузырём отправки - нужно чтобы написанное сразу показывалось».
  Причина: р.147/149 подняли ПУЗЫРИ, но лента продолжалась под
  клавиатурой (edge-to-edge: окно само не сжимается; bottom 200dp
  считался от низа ЭКРАНА, а оверлей top - над клавиатурой ~290dp).
  Фикс: LazyColumn ленты + .imePadding() - при открытой клавиатуре
  вьюпорт сжимается, 200dp запас отводит место под оверлей; последнее
  сообщение (своё и входящие) видно над пузырями. УРОК (edge-to-edge):
  imePadding нужен КАЖДОМУ элементу, который должен быть над
  клавиатурой, - и панели, и контенту. check82 зелёный.
  v11.74.48 = 0fc9b95, run 35984834411 SUCCESS, latest, APK
  40 500 690 Б, sha256 004ccdb2…1ca.
...
- **2026-09-24 (раунд 149) - v11.74.46: клавиатура не прячет ввод
  ...
  windowSoftInputMode=adjustResize НЕ сжимает окно, нужны insets.
  Фикс: MessageInputBar Surface + .imePadding() (foundation.layout
  импортирован wildcard - отдельный import не нужен; якорь импорта
  дважды не совпал - проверить фактические импорты ПЕРЕД якорем).
  check80 зелёный. v11.74.46 = 09168ae, run 35971276501 SUCCESS,
  latest, APK 40 500 694 Б, sha256 abd274dc…64e.

- **2026-09-24 (раунд 148) - v11.74.45: единый ввод в личке; список
  тем чистый.** Владелец (скрин списка тем с полем внизу): «в личных
  тоже так же; в группе где список тем - убрать, только внутри тем».
  Личка: MessageInputBar Row -> Column (Surface сохранён); if(inputFocused)
  Row {скрепка; GIF}; BasicTextField fillMaxWidth + onFocusChanged
  (state/decorator/contentReceiver НЕ тронуты - р.42); send -
  TextButton-пузырь fillMaxWidth (белый 0.85, рамка, верт. padding 8).
  Группа: оверлей р.147 обёрнут if (!showTopicsList). ГРАБЛИ: якорь
  send не сошёлся ДВАЖДЫ - в CircularProgressIndicator строка
  `color = ...` не виделаcь в узком awk-окне; УРОК: якорь собирать тем
  же python-скриптом из файла, а не глазами из awk. Восьмой откат базы
  (cce8ced +730 на 860b47b) - тег снят ДО создания релиза (успел),
  /tmp-копии, reset --hard origin/ветка (1ef0830), перекоммит 812ef05
  (+42/-41). check79 зелёный. v11.74.45 = 812ef05, run 35969113796
  SUCCESS, latest, APK 40 500 694 Б, sha256 08fc62bd…1ed.

- **2026-09-24 (раунд 147) - v11.74.44: оверлей поля + видимый
  «Отправить».** Владелец (скрин: «Отправить» за клавиатурой, поле
  только в правой колонке): «пузырь с отправить тоже был виден при
  вводе; поле ввода расширить на левую область... поверх этой области».
  Причины: (1) в манифесте НЕ БЫЛО windowSoftInputMode - система панила
  и прятала низ; (2) поле жило внутри правой колонки. Фикс: manifest
  adjustResize; блок пилюля+поле -> root Box после Scaffold,
  align(BottomCenter).fillMaxWidth().imePadding() (импорт
  foundation.layout.imePadding); feed contentPadding bottom 200dp;
  стейты ленты подняты на уровень функции. ГРАБЛИ check76: оверлей
  за Box (align не в BoxScope -> Unresolved align) + feedRemaining/
  feedScope в скоупе колонки -> перенос: стейты перед Box, оверлей
  перед закрывающей Box (якорь - «    }» перед «// Раунд 143»).
  check77 зелёный. v11.74.44 = 3f2bb3b, run 35965422878 SUCCESS,
  latest, APK 40 500 694 Б, sha256 55439571…6ab.

- **2026-09-24 (раунд 146) - v11.74.43: поле во всю ширину + «Отправить»
  пузырём.** Владелец (скрин р.145): «поле ввода взять ещё шире левее до
  края экрана; кнопку отправить перемещать под поле ввода своим
  отдельным пузырём той же ширины; правую сторону тоже до конца края».
  Реализация: внешний Column (только вертикальный padding 6) с двумя
  пузырями - поле (clip 18, surface 0.55, рамка, padding 6; при фокусе
  ряд скрепка+GIF; TextField fillMaxWidth до 6 строк) и TextButton
  «Отправить» fillMaxWidth с тем же оформлением (цвет текста по
  состоянию). Вставка по номерам строк; якорь конца - ПУСТАЯ строка
  перед «} // else» (assert поправлен). check75 зелёный.
  v11.74.43 = 33e7d87, run 35961233500 SUCCESS, latest,
  APK 40 500 686 Б, sha256 fa4dc118…815.

- **2026-09-24 (раунд 145) - v11.74.42: просторное поле ввода.**
  Владелец (скрин: узкое поле «то маленькое поле»): больше текста;
  при наборе скрепка и GIF - над полем. Реализация: блок поля Row ->
  Column; inputFocused по Modifier.onFocusChanged (import
  androidx.compose.ui.focus.onFocusChanged); if (inputFocused) Row над
  полем со скрепкой+GIF; нижний Row (Alignment.Bottom): TextField
  weight(1f) minLines 1 maxLines 6 + Отправить. Грабли: (1) замена по
  реконструированному якорю НЕ сошлась - заменять большие блоки по
  НОМЕРАМ СТРОК с assert-ами содержимого; (2) python: """
  + GIF + """.split() - split применился к последнему литералу
  (str+list TypeError) - брать скобки и .split у всего выражения;
  (3) хвост старой закрывающей скобки после вставки по диапазону -
  struct_check поймал «} без открывающей». check74 зелёный.
  v11.74.42 = 99da286, run 35957925589 SUCCESS, latest,
  APK 40 500 046 Б, sha256 30fc8e96…787.

- **2026-09-24 (раунд 144) - v11.74.41: вход в тему на непрочитанном.**
  Владелец: «когда открываешь тему чтобы попадало именно на это
  сообщение, а ниже уже показывало непрочитанные и сколько осталось».
  Причина «верха переписки»: лента - обычный LazyColumn без state.
  Решение: peekTopicUnread() ДО markRead (счётчик сбрасывается при
  первом выпуске ленты!); VM: FeedJump + jumpCaptured - захват один раз
  на смену темы, index = list.size - unread; экран: scrollToItem с
  offset=1 при more-comments, пилюля «↓ Ещё N сообщений» = derivedStateOf
  (visibleItemsInfo.last().index vs size-1), тап - animateScrollToItem.
  remember(derivedStateOf) - ТОЛЬКО с keys (messages.size, moreComments),
  иначе застревает первая лямбда. Шестой откат базы (сброс рабочего
  дерева при reset в recovery) - коммитить правки СРАЗУ, /tmp-копии
  спасли. check73 зелёный. v11.74.41 = bbf2050, run 35955092063
  SUCCESS, latest, APK 40 500 046 Б, sha256 6a3f2707…e05.
...
- **2026-09-24 (раунд 142) - v11.74.39: служебная плашка спрятана.**
  ...
  использовал ОБЩИЙ канал CHANNEL_ID (IMPORTANCE_HIGH) вместе с
  сообщениями. Фикс: новый CHANNEL_SERVICE_ID = apu_service_status
  (IMPORTANCE_MIN, no badge), buildNotification -> служебный канал +
  PRIORITY_MIN + setShowWhen(false); CHANNEL_ID переименован в
  «Сообщения» (имя канала менять можно, важность - нет). Совсем скрыть
  FGS-плашку Android не даёт - объяснено владельцу честно. Грабля
  отката стрельнула 4-й раз: правки (незакоммиченные) стёрлись при
  reset --hard в guard-команде - ПРАВИЛО: guard-скрипт не должен
  делать reset, если в дереве есть правки; правки - сразу коммитить
  до пауз. check70 зелёный с первого раза. v11.74.39 = 6324cef,
  run 35936263924 SUCCESS, latest, APK 40 483 666 Б,
  sha256 4e111c86…f0cc.
...
- **2026-09-23 (раунд 140) - v11.74.37: конверты в запасном пути и
  ...
  ветки Message и Other) сохранял всё в чат БЕЗ фильтров - живой путь
  перехватывает конверты, запасной нет. Фикс: routeIncomingEnvelope -
  копия цепочки стражей основного пути (файлы/гифк-ссылки/гифки/
  стикеры/группы/реакции/удаления/просмотры/сердечки/азбука/прочтения/
  рефералки/звонки), вызов ДО saveIncomingMessage; известный отправитель
  - конверт исполняется, неизвестный - чат не создаётся. (2) Тап по
  уведомлению вёл на главную: в NavGraph onChatLinkConsumed() был
  ПЕРВОЙ строкой LaunchedEffect(pendingChatLink) - обнулял ключ,
  корутина отменялась на первом suspend (запрос в базу), navigate не
  случался. Фикс: cancelChatNotifications -> navigate ->
  onChatLinkConsumed последней. ГРАБЛЯ ДНЯ (дважды!): пересборка
  песочницы откатывает HEAD к 860b47b даже ПОСЛЕ успеха в середине
  раунда (в паузе на ответ владельца); перед ЛЮБЫМ git-действием после
  паузы проверять HEAD, тег ставить отдельным вызовом с if-guard на
  rev-parse. Из-за первого отката: коммит 1c0d701 на 55 файлов лёг на
  старую базу, push ветки отклонён, check67 сожжён; спасение
  (/tmp-копии, fetch refspec, reset --hard, перекоммит) сработало.
  Из-за второго: тег v11.74.37 ушёл на 860b47b, зомби-билд собрал
  старьё; тег снят до его release-шага, зомби пересоздал тег на
  860b47b - снова снят; СТАРЫЙ DRAFT-релиз «v11.74.37» от 18.09
  (39 МБ) заставлял «Skip auto-build if Release already exists» глушить
  билды - draft удалён, тег перевешан на 8aadcd3, билд 35896132230
  собрался честно. Одинаковый размер APK у соседних релизов - норма
  (выравнивание; .32/.33 и .30/.31 тоже совпадали). v11.74.37 =
  8aadcd3, run 35896132230 SUCCESS, latest, APK 40 484 038 Б,
  sha256 3dc0f93f…d34.

- **2026-09-23 (раунд 139) - v11.74.36: стикеры живут в рое, как
  гифки.** Владелец: «Эмодзи и стикеры нужно сделать также как с гиф
  чтобы сохранялись в рое в нашей сети». Эмодзи-смайлы - встроенные
  символы; владельцу предложено три варианта, выбран «эмодзи не трогать».
  Сделано зеркально гифкам (р.121-129): конверты APUSTK1
  (ask/have/want/thumb/thmb) личными E2E-сообщениями, разбор ДО
  сохранения в чат; каталоги хранителей peer_catalog.v1 (100 телефонов
  x 2000 записей), слияние по sha256 минус свои; раздел «Из сети» в
  панели стикеров с миниатюрами t<sha>.jpg (заглушка, пока не доехала);
  выбор стикера - requestSticker ТРЁМ лучшим хранителям (PeerRating),
  байты тихо (isWanted -> без пузыря), оседание в библиотеку
  (addBytes), arrivalsFlow -> автоотправка (личка - картинкой, группа -
  приложением, нужна выбранная тема); тротлимбы: ann 10 мин/телефон +
  fingerprint + 6ч, ask 24ч, want 2 мин, thumb 5 мин. StickerLibrary:
  companion - провод/хотящие/миниатюры/каталоги (как object у гифок),
  экземпляр - all/add/addBytes/entryOf/touch/recents + instance
  syncWithSwarm/buildHaveBatches (зову у VM и сервиса). Грабли:
  companion-функцию нельзя звать через ЭКЗЕМПЛЯР в этом компиляторе
  (e: Unresolved isWanted) - только через имя класса; LocalContext.current
  не композабелен внутри LazyGridScope (только в item{}/верх тела
  секции); Modifier.padding(horizontal=.., bottom=..) - нет перегрузки.
  check64/65 красные, check66 зелёный. v11.74.36 = 44c5b2c,
  run 35877607797 SUCCESS, latest, APK 40 484 038 Б,
  sha256 710c72b5…404.

- **2026-09-23 (раунд 138) - v11.74.35: единая панель ввода
  «Эмодзи / Гиф / Стикеры».** Владелец прислал скрин панели стикеров из
  привычного мессенджера: «чтобы при нажатие на кнопку появлялось облако
  с эмодзи, гиф и стикеры в одном пузыре... и чтобы когда листаешь
  выбираешь... вверху также сделать пузыри разделов... листающиеся
  горизонтально». Сделано: кнопка гифок в личке/группах/комментариях
  открывает InputPanelDialog - пузырь APU-стиля (золотая рамка,
  ChatWallpaper) с тремя пузырями разделов; содержимое раздела - сетка +
  горизонтальная лента пузырей подразделов (SubPillRow), активный
  подсвечивается по firstVisibleItemIndex (derivedStateOf) и прокручивает
  сетку по нажатию (scrollToItem на префиксных суммах высот).
  Эмодзи: EmojiCatalog (7 категорий), вставка в поле ввода (личка -
  onInputTextChanged(inputText+emoji), группы - draft). Гиф: тело
  GifCatalogDialog вынесено в публичный GifCatalogBody и встроено
  разделом; старый диалог - обёртка (совместимость). Стикеры:
  StickerLibrary (@Singleton, noBackupFilesDir/stickers/v1; index.v1
  APUSTICK1: sha|b64url(name)|atMs, файлы <sha>.img, дедуп по sha256,
  атомарная запись tmp+move; recents.v1 APUSTICKREC1, cap 24). Разделы
  «Недавние»/«Мои», «+» = GetContent("image/*"). Отправка: личка -
  ChatDetailViewModel.sendSticker -> filePreparation.prepareFromFile
  (картинкой сразу), группы - FileProvider + groupFiles.stage +
  GroupFileMarker.compose + sendMessage; после - touch + refreshStickers.
  Роевой каталог стикеров - отдельная задача (не в этом раунде).
  check63 зелёный. v11.74.35 = badb001, run 35865127108 SUCCESS, latest,
  APK 40 451 270 Б, sha256 bdb37d0f…63c.

- **2026-09-23 (раунд 137) - v11.74.34: «удалить у всех» доезжает
  всегда.** Владелец: «ждёт когда появятся все онлайн - не удобно...
  команду отправил и дальше эта команда должна выполняться через тех
  кто держится в сети а если нет никого в сети то хранится на телефонах
  и как дойдёт до места удаления». Сделано: очередь DeletionOutbox
  (файл noBackupFilesDir/deletion_outbox/v1, APUDELQ1, TTL 7 дней,
  попытка раз в 10 мин из помпы сервиса; из CoreServerService зовутся
  messageDeletion.pump() + groupRepository.pumpDeletions()). Личка:
  APUDEL1 уходит сразу + запись в очереди до ACK получателя
  («APUDEL1|ack|msgId» -> removeByTarget); офлайн-собеседник получает
  удаление при появлении. Группы: msdel v2 несёт deleterId
  (buildMessageDelete(groupId, msgId, deleterId); форма р135 с 4 полями
  принимается, deleter=sender); приёмник проверяет права по deleter
  (автор/владелец), РЕТРАНСЛИРУЕТ троим случайным соседям (раз в час на
  удаление, память deletionRelayedAt, fanout 3) и кладёт в свою очередь;
  помпа репозитория досылает двум непробованным за круг; вышедшие из
