...
- **M3(a) ВЕРИФИЦИРОВАН на 3 телефонах:** `relay|…` разбирается через `wire::parse`;
  ...
  Автоматическая отправка receipt, gossip и send-path ещё НЕ добавлены.
...
**Следующий шаг = автоматическая отправка receipt получателем; M3(c) пока не начинать.**
...
- **2026-08-14 (доп.)** — M3(b) собран и проверен на 3 телефонах: relay→ручной receipt
  ...
  Следующий обязательный мини-шаг до gossip — автоматическая отправка receipt получателем.
...
4. До M3(c) добавить отдельным мини-шагом автоматическую отправку mesh receipt получателем
   relay-сообщения (ручной receipt уже доказал обработку). Gossip/send-path пока не трогать.
   Перед каждым будущим `enqueue` обязателен `contains(msg_id)`.
