...
- **M3(a), код собран:** `relay|…` разбирается через `wire::parse`; локальный получатель
  получает `MessageReceived`, чужое сообщение хранится с TTL/hop. Перед единственным
  `enqueue` явно вызывается `contains(msg_id)`. Windows `build-rust.ps1` успешно завершён
  2026-08-14. Receipt/gossip/send-path ещё НЕ добавлены.

**Следующий шаг = телефонный тест M3(a) через ручной relay-конверт; M3(b) пока не начинать.**
...
- **2026-08-14** — M3(a) успешно собран на Windows (`build-rust.ps1`, arm64-v8a). Следующий
  шаг — APK и управляемый тест ручного `relay`-конверта на телефонах; до него M3(b) не начинать.
...
3. Проверить, что текущая ветка — `arena/019ffc32-apumir`. M3.1 и M3(a) уже успешно
   собраны через Windows `build-rust.ps1`.
4. Следующий шаг — собрать APK и провести управляемый телефонный тест ручного
   `relay`-конверта. M3(b) начинать только после успешного теста и отдельного «да».
   Receipt/gossip/send-path пока не трогать. Перед каждым `enqueue` обязателен `contains(msg_id)`.
