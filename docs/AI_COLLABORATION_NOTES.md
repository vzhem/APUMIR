...
- 2026-08-26 (РЕЛИЗ v11.19.0): владелец явно разрешил выпуск и выбрал версию
  ...
  workflows.
  Тег v11.19.0 указывает на d47db72, который отличается от проверенного гейтом
  7170c53 только скриптом make-release.ps1 (git diff --stat - один файл); код
  приложения идентичен.
  Сборку и публикацию делает GitHub Actions по тегу, публикация - pre-release,
  перевод в полноценный релиз остаётся ручным действием владельца.
