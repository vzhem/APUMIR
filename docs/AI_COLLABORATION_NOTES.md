...
- **M3(c.1), код собран:** при presence peer получает адресный `gsumm`; принимать его может
  ...
  Windows `build-rust.ps1` успешно завершён 2026-08-14.

**Следующий шаг = APK + телефонный тест `gsumm`; M3(c.2) пока не начинать.**
...
- **2026-08-14 (доп.3)** — M3(c.1) собран (`build-rust.ps1`): адресная отправка/приём
  `gsumm` без relay, с лимитами summary/cooldown/global budget. Зафиксировано честно:
  глобальный MQTT wildcard не масштабируется на тысячи; до M3(c.2) нужны relay batch/byte
  budget и bounded fanout. Пользователь задал relay-приоритет: друзья → друзья друзей →
  любые узлы только при свободных ресурсах телефона.
...
4. M3(c.1) адресных `gsumm` собран с item/byte/cooldown/global limits, но без relay.
   Следующий шаг — APK + телефонный тест логов. M3(c.2) не начинать до «да»; перед кодом
   согласовать batch/byte budget, bounded fanout и веса friend/FoF/idle-public. M3(d) не трогать.
