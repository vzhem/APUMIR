...
$Script    = "p2p-relay"
$WorkerUrl = "https://raw.githubusercontent.com/vzhem/APUMIR/38368c0edf5394c8da341ee837e7e24025012c8b/tools/worker/p2p_relay_worker.js"
...
if (Select-String -Path "worker.js" -Pattern "gif/search" -Quiet) {
  Write-Host "  OK: /gif/search endpoint present (GIF catalog)"
} else {
  Write-Host "ERROR: downloaded worker.js has NO /gif/search endpoint - the script URL and the worker tag are out of sync." -ForegroundColor Red
  Write-Host "Deploy aborted. Fetch the deploy script from the same tag/commit as the worker and retry." -ForegroundColor Red
  exit 1
}

# Optional GIF catalog key (Tenor). Create the free key at
# https://console.cloud.google.com (enable "Tenor API", create API key),
# save it to tenor-key.txt next to this script and re-run the deploy.
# Without the key everything works except the GIF catalog search.
$tenorBindings = @()
if (Test-Path "tenor-key.txt") {
  $tenorKey = (Get-Content "tenor-key.txt" -Raw).Trim()
  if ($tenorKey) {
    $tenorBindings = @(@{ type = "plain_text"; name = "TENOR_KEY"; text = $tenorKey })
